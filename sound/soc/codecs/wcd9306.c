	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	unsigned int dmic;
	int ret;
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2013/10/15, Fix the platform bug about dmic control*/
	...
	ret = kstrtouint(strpbrk(dmic_name, "1234"), 10, &dmic);
	if (ret < 0) {
                        ...
                        int event)
{
        ...
        switch (event) {
        case SND_SOC_DAPM_PRE_PMU:
                tapan->mbhc.mbhc_cfg->mclk_cb_fn(codec, 1, true);
                tapan_codec_enable_ldo_h(w, kcontrol, event);
                tapan_codec_enable_micbias(w, kcontrol, event);
                break;
        case SND_SOC_DAPM_POST_PMU:
                tapan->mbhc.mbhc_cfg->mclk_cb_fn(codec, 0, true);
                break;
        case SND_SOC_DAPM_POST_PMD:
                tapan_codec_enable_micbias(w, kcontrol, event);
                tapan_codec_enable_ldo_h(w, kcontrol, event);
...
#define GPIO_HSPA_SW 12
#define GPIO_HSPA_SD_N 114
static bool g_is_headset_pa_on = false;
static int  g_headset_pa_current_gain = 0;
extern struct qpnp_vadc_chip *g_vadc_dev;
...
static int tpa6130a2_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		printk("[tpa6130a2]%s: i2c check functionality error\n", __func__);
		return -ENODEV;
	}
...
MODULE_DEVICE_TABLE(i2c, tpa6130a2_i2c_id);
...
#else
#define tpa6130a2_match_table NULL
#endif

static const struct i2c_device_id tpa6130a2_i2c_id[] = {
     { "tpa6130a", 0 },
     { }
};
...
int headset_vadc_read(void)
{
	int rc;
	struct qpnp_vadc_result results;

	gpio_direction_output(GPIO_HSPA_SW, 0);
	msleep(10);

	rc = qpnp_vadc_read(g_vadc_dev, P_MUX7_1_3, &results);
	if (rc)
		printk("Unable to read headset impedandce rc=%d\n", rc);

	gpio_direction_output(GPIO_HSPA_SW, 1);

	return (int)results.physical / 1000;
}

void tpa6130a2_turn_on(bool on)
{
	if (on) {
		gpio_direction_output(GPIO_HSPA_SD_N, 1);
	}
	else {
		gpio_direction_output(GPIO_HSPA_SD_N, 0);
	}
	return;
}

void tpa6130a2_enable_output(bool enable)
{
	char data[2] = {0x1, 0xc0};

	if (!enable)
		data[1] = 0x0;

	i2c_master_send(g_client, data, 2);

	return;
}

void tpa6130a2_set_gain(int gain)
{
	char data[2] = {0x2, 0x0};

	data[1] = gain;
	g_headset_pa_current_gain = gain;
	i2c_master_send(g_client, data, 2);
}

void tpa6130a2_enable_HiZ(bool enable)
{
	char data[2] = {0x3, 0x3};
...
bool is_tpa6130a2_enabled(void)
{
	return g_is_headset_pa_on;
}
#endif
/*[PLATFORM]-Add-END by TCTNB.WQF*/
...
/*[PLATFORM]-Add-BGEIN by TCTNB.WQF,2013/11/11, add headset pa support.*/
#ifdef CONFIG_SND_SOC_HS_TPA6130A2
		if (!g_is_headset_pa_on) {
			pr_debug("Enable headset pa\n");
			tpa6130a2_turn_on(true);
			tpa6130a2_set_gain(g_headset_pa_current_gain);
			tpa6130a2_enable_HiZ(false);
			tpa6130a2_enable_output(true);
			g_is_headset_pa_on = true;
		}
#endif
/*[PLATFORM]-Add-END by TCTNB.WQF*/
...
#ifdef CONFIG_SND_SOC_HS_TPA6130A2
		if (g_is_headset_pa_on) {
			pr_debug("Disable headset pa\n");
			tpa6130a2_enable_output(false);
			tpa6130a2_enable_HiZ(true);
			tpa6130a2_turn_on(false);
			g_is_headset_pa_on = false;
		}
#endif
/*[PLATFORM]-Add-END by TCTNB.WQF*/
		break;
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2013/12/27, No need to print this log as normal.*/
#ifdef CONFIG_TCT_8X26_COMMON
	pr_debug("%s: event = %d\n", __func__, event);
#else
	pr_info("%s: event = %d\n", __func__, event);
...
/*[PLATFORM]-Add-BGEIN by TCTNB.WQF,2013/11/11, add headset pa support.*/
#ifdef CONFIG_SND_SOC_HS_TPA6130A2
	i2c_add_driver(&tpa6130a2_i2c_driver);

	ret = gpio_request(GPIO_HSPA_SW, "HSPA_SW");
	if (ret)
		printk("HSPA_SW gpio request failed\n");
	ret = gpio_direction_output(GPIO_HSPA_SW, 1);

	ret = gpio_request(GPIO_HSPA_SD_N, "HSPA_SD_N");
	if (ret)
		printk("HSPA_SD_N gpio request failed\n");
	ret = gpio_direction_output(GPIO_HSPA_SD_N, 1);
