...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2013/9/2, Support iphone headset detection*/
#ifdef CONFIG_TCT_8X26_COMMON
#define WCD9XXX_V_CS_HS_MAX 2450
#else
...
{
/*[PLATFORM]-Add-BGEIN by TCTNB.WQF,2013/12/17, add headset pa support.*/
#ifdef CONFIG_SND_SOC_HS_TPA6130A2
	unsigned int headset_vadc;
	int headset_gain_db;
	unsigned int headset_gain_value;
#endif
/*[PLATFORM]-Add-END by TCTNB.WQF*/
...
/*[PLATFORM]-Add-BGEIN by TCTNB.WQF,2013/11/11, add headset pa support.*/
	...
	if (mask == WCD9XXX_JACK_MASK) {
		if (status == 0) {
			...
			tpa6130a2_enable_HiZ(true);
			headset_vadc = headset_vadc_read();
			headset_gain_db = headset_vadc/10 - 140;
			printk("Headset impedance = %d, gain = %d(DB*10)\n", headset_vadc, headset_gain_db);

			headset_gain_value = 0x35 + headset_gain_db/5;
			if (headset_gain_value < 0x1d)  //minimum gain is -13db
				headset_gain_value = 0x1d;
			else if (headset_gain_value > 0x2c)  //maxmum gain is -4.1db
				headset_gain_value = 0x2c;
			if (headset_vadc > 1600)       // if high impedance headset
				headset_gain_value = 0x2a; //maxmum gain is -5.1db
			printk("Headset gain value = 0x%x\n", headset_gain_value);
			tpa6130a2_set_gain(headset_gain_value);
			if (is_tpa6130a2_enabled()) {
				printk("Enable headset pa when insert\n");
...
#ifdef CONFIG_TCT_8X26_COMMON
void tapan_codec_enable_mbhc_micbias(struct wcd9xxx_mbhc *mbhc, bool enable)
{
	int ret;

	if (!mbhc->mbhc_cfg->micbias_always_on)
		return;
...
/*[PLATFORM]-Add-BGEIN by TCTNB.WQF,2014/1/8, set micbias always high after headset insert.*/
		...
		if (jack_type == SND_JACK_HEADSET)
			tapan_codec_enable_mbhc_micbias(mbhc, false);
#endif
/*[PLATFORM]-Add-END by TCTNB.WQF*/
...
/*[PLATFORM]-Add-BGEIN by TCTNB.WQF,2014/1/8, set micbias always high after headset insert.*/
#ifdef CONFIG_TCT_8X26_COMMON
			tapan_codec_enable_mbhc_micbias(mbhc, true);
#endif
/*[PLATFORM]-Add-END by TCTNB.WQF*/
...
/*[PLATFORM]-Add-BGEIN by TCTNB.WQF,2013/12/26, for PR:557658, enable Vortex moshi headset.*/
#ifdef CONFIG_TCT_8X26_COMMON
			if (!(d->_vdces >= 192 && d->_vdces <= 208)) {
#endif
...
/*[PLATFORM]-Add-BGEIN by TCTNB.WQF,2013/12/24, rio6 can't detect headset well when play sound, so ignore hphl_status check*/
#if defined(CONFIG_TCT_8X26_RIO6)||defined(CONFIG_TCT_8X26_RIO6_TF)
		      type == PLUG_TYPE_HEADPHONE) && (ch != sz && ch > 0))) {
#else
...
/*[PLATFORM]-Add-BGEIN by TCTNB.WQF,2013/12/5, Fix platform bug for headset detect.*/
#ifndef CONFIG_TCT_8X26_COMMON
	/* cancel detect plug */
	wcd9xxx_cancel_hs_detect_plug(mbhc,
				      &mbhc->correct_plug_swch);
#endif
...
/*[PLATFORM]-Add-BGEIN by TCTNB.WQF,2014/1/3, Don't disable irq when suspend.*/
#ifdef CONFIG_TCT_8X26_COMMON
					    IRQF_NO_SUSPEND |
#endif
/*[PLATFORM]-Add-END by TCTNB.WQF*/
				...
				void *data)
{
	...
	case WCD9XXX_EVENT_POST_MICBIAS_4_OFF:
		if (mbhc->mbhc_cfg && mbhc->mbhc_cfg->micbias ==
			 ...
			 */
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2013/11/20, Fix platform bug of hook key detection*/
#ifdef CONFIG_TCT_8X26_COMMON
			if (!mbhc->polling_active &&
				mbhc->current_plug != PLUG_TYPE_GND_MIC_SWAP &&
				!mbhc->current_plug != PLUG_TYPE_HEADSET)
