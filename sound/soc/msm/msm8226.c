...
#define GPIO_NXP9890_RESET    4
#endif
/*[PLATFORM]-Add-END by TCTNB.WQF*/

/*[PLATFORM]-Add-BGEIN by TCTNB.WQF,2013/11/11, add headset pa support.*/
#ifdef CONFIG_SND_SOC_HS_TPA6130A2
#include <linux/qpnp/qpnp-adc.h>
struct qpnp_vadc_chip *g_vadc_dev;
#endif
/*[PLATFORM]-Add-END by TCTNB.WQF*/
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2013/11/25, we only need one hook key of headset */
...
/*[PLATFORM]-Add-BGEIN by TCTNB.WQF,2013/11/11, using gpio for headset detect*/
#if defined(CONFIG_TCT_8X26_EOS)||defined(CONFIG_TCT_8X26_RIO6)||defined(CONFIG_TCT_8X26_RIO6_TF)||defined(CONFIG_TCT_8X26_RIO5_CTCC)
	.gpio = 13,
	.gpio_irq = 610,
#else
	.gpio = 0,
...
/*[PLATFORM]-Add-END by TCTNB.WQF*/
	...
	.use_vddio_meas = true,
/*[PLATFORM]-Add-BGEIN by TCTNB.WQF,2014/1/8, set micbias always high after headset insert.*/
#ifdef CONFIG_TCT_8X26_COMMON
...
//16 bit width and 48 kHz sample rate
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2013/8/21, Add the PA control for HAC function */
#ifdef CONFIG_TCT_8X26_COMMON
static int ext_hac_amp_gpio = -1;
#endif
/*[PLATFORM]-Add-END by TCTNB.WQF*/
...
{
	if (enable) {
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2013/9/25, Set mode 3 for PA of AW8736 */
#ifdef CONFIG_TCT_8X26_COMMON
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2013/8/21, Add the PA control for HAC function*/
#ifdef CONFIG_TCT_8X26_COMMON
static bool is_hac_enabled = false;

static int msm8226_get_hac(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = is_hac_enabled;
	return 0;
}
static int msm8226_set_hac(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	if (is_hac_enabled == ucontrol->value.integer.value[0])
		return 0;

	is_hac_enabled = ucontrol->value.integer.value[0];
	gpio_direction_output(ext_hac_amp_gpio, is_hac_enabled);
	printk("%s: %d\n", __func__, is_hac_enabled);
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2013/8/21, Add the PA control for HAC function*/
#ifdef CONFIG_TCT_8X26_COMMON
	SOC_ENUM_EXT("HAC PA Enable", msm_snd_enum[sizeof(msm_snd_enum)/sizeof(struct soc_enum) - 1], msm8226_get_hac,
		msm8226_set_hac),
#endif
/*[PLATFORM]-Add-END by TCTNB.WQF*/
};
...
#define S(X, Y) ((WCD9XXX_MBHC_CAL_PLUG_TYPE_PTR(tapan_cal)->X) = (Y))
	S(v_no_mic, 30);
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2013/11/19, Modify hs max for iphone headset support */
#ifdef CONFIG_TCT_8X26_COMMON
	S(v_hs_max, 2700);
#else
	S(v_hs_max, 2450);
...
/*[PLATFORM]-Add-BGEIN by TCTNB.WQF,2013/11/11, add headset pa support.*/
#ifdef CONFIG_SND_SOC_HS_TPA6130A2
	g_vadc_dev = qpnp_get_vadc(&pdev->dev, "headset");
#endif
/*[PLATFORM]-Add-END by TCTNB.WQF*/
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2014/1/17, config reset gpios for NXP9890*/
#ifdef CONFIG_SND_SOC_NXP9890
	ret = gpio_request(GPIO_NXP9890_RESET, "nxp9890_reset");
	if (ret)
		printk("nxp9890_reset gpio request failed\n");
	ret = gpio_direction_output(GPIO_NXP9890_RESET, 1);
	msleep(2);
	ret = gpio_direction_output(GPIO_NXP9890_RESET, 0);
#endif
/*[PLATFORM]-Add-END by TCTNB.WQF*/
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2013/8/21, Add the PA control for HAC function*/
#ifdef CONFIG_TCT_8X26_COMMON
#ifdef CONFIG_TCT_8X26_EOS
		ext_hac_amp_gpio = of_get_named_gpio(pdev->dev.of_node,
					"qcom,cdc-hac-gpios", 0); // No HAC function on pop7lte changshun.zhou 20140211
#else
        ext_hac_amp_gpio = -1; // No HAC function on pop7lte changshun.zhou 20140211
#endif
			if (ext_hac_amp_gpio < 0) {
				dev_err(&pdev->dev,
					"Looking up %s property in node %s failed %d\n",
					"qcom,cdc-hac-gpios",
					pdev->dev.of_node->full_name, ext_hac_amp_gpio);
			} else {
				ret = gpio_request(ext_hac_amp_gpio,
						"TAPAN_CODEC_HAC_AMP");
				if (ret) {
					/* GPIO to enable EXT AMP exists, but failed request */
					dev_err(card->dev,
						"%s: Failed to request tapan hac amp gpio %d\n",
						__func__, ext_hac_amp_gpio);
					goto err_vdd_spkr;
				}
			}
#endif
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2014/3/18, Disable the PA default*/
#ifdef CONFIG_TCT_8X26_COMMON
		gpio_direction_output(ext_spk_amp_gpio, 0);
#endif
/*[PLATFORM]-Add-END by TCTNB.WQF*/
