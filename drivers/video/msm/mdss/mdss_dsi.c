...
/* [PLATFORM]-Mod-BEGIN by TCTNB.FLF, PR-582255, 2014/01/10, set powerIC Vp/n to 5.5v */
...
/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, PR-571060, 2013/12/20, add board_id detect for rio6 */
#if defined(CONFIG_TCT_8X26_RIO6)||defined(CONFIG_TCT_8X26_RIO6_TF)
...
#define GPIO_BOARD_ID0			66
#define GPIO_BOARD_ID1			67
#define GPIO_BOARD_ID2			52
/*
 * board_id_detect: detect board id by check gpio66, gpio67, gpio52
 * param: none
 * return value: integer made up of value read from gpio66, gpio67, gpio52 or -ENODEV
 *				 [gpio52][gpio67][gpio66]
 */
int board_id_detect(void)
{
	...
	rc = gpio_request(GPIO_BOARD_ID0, "board_id0");
	if (rc) {
		pr_err("request gpio66 failed, rc=%d\n", rc);
		goto err_gpio66;
	}
	...
	if (rc) {
		pr_err("%s: unable to config tlmm = %d\n",
			__func__, GPIO_BOARD_ID0);
		goto err_gpio66;
	}

	rc = gpio_direction_input(GPIO_BOARD_ID0);
	if (rc) {
		pr_err("set_direction for gpio66 failed, rc=%d\n",
		       rc);
		goto err_gpio66;
	}

	rc = gpio_request(GPIO_BOARD_ID1, "board_id1");
	if (rc) {
		pr_err("request gpio67 failed, rc=%d\n", rc);
		goto err_gpio67;
	}
	...
	if (rc) {
		pr_err("%s: unable to config tlmm = %d\n",
			__func__, GPIO_BOARD_ID1);
		goto err_gpio67;
	}

	rc = gpio_direction_input(GPIO_BOARD_ID1);
	if (rc) {
		pr_err("set_direction for gpio67 failed, rc=%d\n",
		       rc);
		goto err_gpio67;
	}

	rc = gpio_request(GPIO_BOARD_ID2, "board_id2");
	if (rc) {
		pr_err("request gpio52 failed, rc=%d\n", rc);
		goto err_gpio52;
	}
	...
	if (rc) {
		pr_err("%s: unable to config tlmm = %d\n",
			__func__, GPIO_BOARD_ID2);
		goto err_gpio52;
	}

	rc = gpio_direction_input(GPIO_BOARD_ID2);
	if (rc) {
		pr_err("set_direction for gpio52 failed, rc=%d\n",
		       rc);
		goto err_gpio52;
	}

	sprintf(board_id, "%d%d%d", gpio_get_value(GPIO_BOARD_ID2), gpio_get_value(GPIO_BOARD_ID1), gpio_get_value(GPIO_BOARD_ID0));

	gpio_free(GPIO_BOARD_ID0);
	gpio_free(GPIO_BOARD_ID1);
	gpio_free(GPIO_BOARD_ID2);
	printk("%s: board_id is \"%s\", id = %d\n", __func__, board_id, atoi(board_id));
...
EXPORT_SYMBOL(board_id_detect);
#endif
/* [PLATFORM]-Add-END by TCTNB.FLF */
/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, FR-548679, 2013/11/25, add ic 98607 to power on hx8394-A*/
#if defined(CONFIG_TCT_8X26_RIO6)||defined(CONFIG_TCT_8X26_RIO6_TF)
static int gpio_lcd_pwr_enp = -1;
static int gpio_lcd_pwr_enn = -1;
static int il98607_init(void)
{
	int err;
	err = board_id_detect();
	if (err == 0) {
		pr_err("mockup board detected!\n");
		gpio_lcd_pwr_enp = 4;
		gpio_lcd_pwr_enn = 5;
	} else if (err == -ENODEV) {
		pr_err("error board detected!\n");
		goto err_detect;
	} else {
		pr_err("proto/PIO board detected!\n");
		gpio_lcd_pwr_enp = 110;
		gpio_lcd_pwr_enn = 111;
	}
	...
	err = gpio_tlmm_config(GPIO_CFG(gpio_lcd_pwr_enp, 0, GPIO_CFG_OUTPUT,
								  GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (err) {
		pr_err("%s: display_pwron_gpiop config failed\n", __func__);
		goto err_gpio_enp;
	}
	...
	err = gpio_tlmm_config(GPIO_CFG(gpio_lcd_pwr_enn, 0, GPIO_CFG_OUTPUT,
								  GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (err) {
		pr_err("%s: display_pwron_gpion config failed\n", __func__);
		goto err_gpio_enn;
	}
...
//zrl add for porting POP7 TDT LCD NT51018_52003,140111,begin
//#define LCD_POWER_EN	69
//#define LCDC_DISP		67 // modify by changshun.zhou for white screen after wake up 20140129
//#define POP7_RESET		25
//zrl add for porting POP7 TDT LCD NT51018_52003,140111,end

static int mdss_dsi_panel_power_on(struct mdss_panel_data *pdata, int enable)
{
	int ret = 0;	//zrl add for porting POP7 TDT LCD NT51018_52003,140111
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	...
	if (enable) {
		...
		// modify by changshun.zhou enable vreg for the screen refresh abnormal 20140306 begin
		...
		// modify by changshun.zhou enable vreg for the screen refresh abnormal 20140306 end


/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, PR-548695, 2013/12/05, add lcd nt35512 for rio4g */
#if !defined(CONFIG_TCT_8X26_RIO6) && !defined(CONFIG_TCT_8X26_RIO6_TF)
...
/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, FR-548679, 2013/11/25, add ic 98607 to power on hx8394-A*/
	...
	if(1 == lcd_no_poweric)
        if (pdata->panel_info.panel_power_on == 0) {
            mdelay(5);
            printk("%s: set gpio_lcd_pwr_enp value = 1\n", __func__);
            gpio_set_value(gpio_lcd_pwr_enp, 1);
            mdelay(10);
            printk("%s: set gpio_lcd_pwr_enn value = 1\n", __func__);
            gpio_set_value(gpio_lcd_pwr_enn, 1);
            mdelay(10);
        }
#endif
/* [PLATFORM]-Add-END by TCTNB.FLF */
...
/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, FR-548679, 2013/11/25, add ic 98607 to power on hx8394-A*/
	...
	if(1 == lcd_no_poweric) {
        mdelay(5);
        printk("%s: set gpio_lcd_pwr_enn value = 0\n", __func__);
        gpio_set_value(gpio_lcd_pwr_enn, 0);
        mdelay(10);
        printk("%s: set gpio_lcd_pwr_enp value = 0\n", __func__);
        gpio_set_value(gpio_lcd_pwr_enp, 0);
	}
#endif
/* [PLATFORM]-Add-END by TCTNB.FLF */
		// modify by changshun.zhou disable vreg for the screen refresh abnormal 20140306 begin
		ret = msm_dss_enable_vreg(
			ctrl_pdata->power_data.vreg_config,
...
/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, PR-583186, 2014/01/02, enable external power supply for rio6 lcd */
#if defined(CONFIG_TCT_8X26_RIO6)||defined(CONFIG_TCT_8X26_RIO6_TF)
	if(1 == lcd_no_poweric)
        if (pdata->panel_info.panel_power_on == 0) {
            mdelay(5);
...
#if defined(CONFIG_TCT_8X26_RIO6)||defined(CONFIG_TCT_8X26_RIO6_TF)
/*added to resolve that lcd display error(black...) for rio6 global/tf  2014.03.05*/
		schedule_delayed_work(&ctrl_pdata->reset_panel_timer, msecs_to_jiffies(3000));
/*added end*/
#endif
	}
...
#if defined(CONFIG_TCT_8X26_RIO6)||defined(CONFIG_TCT_8X26_RIO6_TF)
/*added to resolve that lcd display error(black...) for rio6 global/tf  2014.03.05*/
		cancel_delayed_work_sync(&ctrl_pdata->reset_panel_timer);
/*added end*/
#endif
	}
	pr_debug("%s-:End\n", __func__);
...
/* [PLATFORM]-Mod-BEGIN by TCTNB.FLF, PR-582255, 2014/01/10, set powerIC Vp/n to 5.5v */
#if defined(CONFIG_TCT_8X26_RIO6)||defined(CONFIG_TCT_8X26_RIO6_TF)
static struct i2c_client *pwric_client;
static void set_pwric(void)
{
    int ret = 0;
	char data[2] = {0x06, 0x0A};

	ret = i2c_master_send(pwric_client, data, 2);
	if (ret != 2)
		pr_err("%s: (%d)pwrIC set VBST_OUT failed!\n", __func__, ret);
    data[0] = 0x09;
    data[1] = 0x0A;
	ret = i2c_master_send(pwric_client, data, 2);
	if (ret != 2)
		pr_err("%s: (%d)pwrIC set VP_OUT failed!\n", __func__, ret);

    data[0] = 0x08;
    data[1] = 0x0A;
	ret = i2c_master_send(pwric_client, data, 2);
	if (ret != 2)
		pr_err("%s: (%d)pwrIC set VN_OUT failed!\n", __func__, ret);
}
static int __devinit pwric_probe(struct i2c_client *client,
			   const struct i2c_device_id *id)
{
	pwric_client = client;
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		printk("[pwric]%s: i2c check functionality error\n", __func__);
		return -ENODEV;
	}
    set_pwric();
    return 0;
...
#if defined(CONFIG_TCT_8X26_RIO6)||defined(CONFIG_TCT_8X26_RIO6_TF)
/*added to resolve that lcd display error(black...) for rio6 global/tf  2014.03.05*/
static void mipi_dsi_reset_panel(struct work_struct *work)
{
    struct mdss_dsi_ctrl_pdata *ctrl_pdata = container_of(to_delayed_work(work),
                 struct mdss_dsi_ctrl_pdata, reset_panel_timer);

    int ret = 0;
    ret = ctrl_pdata->reset_timer(&ctrl_pdata->panel_data);
...
/* [PLATFORM]-Add-BEGIN by TCTNB.YJ, 2014/03/10, for rio4g ared id detect */
#ifdef CONFIG_TCT_8X26_RIO4G
#define RIO4G_AREA_ID 56
static struct class * rio4g_devinfo_class;
static struct device * rio4g_info_dev;
static int rio4g_area_id = 2;

static ssize_t rio4g_area_id_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret;
...
static int get_area_id(void)
{
	...
	rc = gpio_request(RIO4G_AREA_ID, "rio4g_area_id");
	if (rc) {
	...
	if (rc) {
		printk(">- %s: unable to config tlmm = %d\n",
			__func__, RIO4G_AREA_ID);
		goto err;
	}

	rc = gpio_direction_input(RIO4G_AREA_ID);
	if (rc) {
		printk(">- set_direction for gpio56 failed, rc=%d\n",
		       rc);
		goto err;
	}
	rio4g_area_id = gpio_get_value(RIO4G_AREA_ID);
	printk(">- area_id = %d \n", rio4g_area_id);

	gpio_free(RIO4G_AREA_ID);
	return 1;
...
static DEVICE_ATTR(area_id_info, 0644, rio4g_area_id_show, NULL);
#endif
...
/* [PLATFORM]-Add-BEGIN by TCTNB.YJ, 2014/03/10, for rio4g ared id detect */
	...
	rio4g_devinfo_class = class_create(THIS_MODULE, "rio4g_devinfo");
    if (IS_ERR(rio4g_devinfo_class))
        pr_err("Failed to create class(lcd_ce)!\n");
    rio4g_info_dev = device_create(rio4g_devinfo_class, NULL, 0, NULL, "area_id");
    if (IS_ERR(rio4g_info_dev))
        pr_err("Failed to create device(lcd_ce_ctrl)!\n");

    // lcd_ce_status
    if (device_create_file(rio4g_info_dev, &dev_attr_area_id_info) < 0)
       pr_err("Failed to create device file(%s)!\n", dev_attr_area_id_info.attr.name);

	dev_set_drvdata(rio4g_info_dev, pdev);
...
if(1 == lcd_no_poweric) {
    rc = il98607_init();
	if (rc) {
		pr_err("%s: dsi panel powerIC control failed\n", __func__);
		return rc;
	}
}
#endif
...
#if defined(CONFIG_TCT_8X26_RIO6)||defined(CONFIG_TCT_8X26_RIO6_TF)
/*added to resolve that lcd display error(black...) for rio6 global/tf  2014.03.05*/
	INIT_DELAYED_WORK(&ctrl_pdata->reset_panel_timer, mipi_dsi_reset_panel);
/*added end*/
#endif
...
#if 0 // modify by changshun.zhou for white screen after wake up 20140129
//zrl add for porting POP7 TDT LCD NT51018_52003,140111,begin	
	pr_err("%s, line:%d zrl debug\n",__func__,__LINE__);
	
	//init LCD_POWER_EN
	rc = gpio_request(LCD_POWER_EN, "display_power_en");
	if (rc) {
		pr_err("%s: LCD_POWER_EN request failed\n", __func__);
	}
	
	rc = gpio_tlmm_config(GPIO_CFG(LCD_POWER_EN, 0, GPIO_CFG_OUTPUT,
								  GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc) {
		pr_err("%s: LCD_POWER_EN config failed\n", __func__);
	}
	
	rc = gpio_direction_output(LCD_POWER_EN, 1);
	if (rc) {
		pr_err("%s: set_direction for LCD_POWER_EN failed\n", __func__);
	}
	
	//init LCDC_DISP
	rc = gpio_request(LCDC_DISP, "display_power_avdd");
	if (rc) {
		pr_err("%s: LCDC_DISP request failed\n", __func__);
	}

	rc = gpio_tlmm_config(GPIO_CFG(LCDC_DISP, 0, GPIO_CFG_OUTPUT,
								  GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc) {
		pr_err("%s: LCDC_DISP config failed\n", __func__);
	}

	rc = gpio_direction_output(LCDC_DISP, 1);
	if (rc) {
		pr_err("%s: set_direction for LCDC_DISP failed\n", __func__);
	}
//zrl add for porting POP7 TDT LCD NT51018_52003,140111,end
#endif
...
/* [PLATFORM]-Mod-BEGIN by TCTNB.FLF, PR-582255, 2014/01/10, set powerIC Vp/n to 5.5v */
#if defined(CONFIG_TCT_8X26_RIO6)||defined(CONFIG_TCT_8X26_RIO6_TF)
static const struct i2c_device_id pwric_id[] = {
	{"pwric", 0},
	{},
};
MODULE_DEVICE_TABLE(i2c, pwric_id);
static struct of_device_id pwric_match_table[] = {
	{ .compatible = "pwric_il98607",},
	{ },
};
static struct i2c_driver pwric_driver = {
	.probe = pwric_probe,
	.driver = {
		   .name = "pwric",
		   .owner = THIS_MODULE,
		   .of_match_table = pwric_match_table,
		   },
	.id_table = pwric_id,
};
#endif
...
#if defined(CONFIG_TCT_8X26_RIO6)||defined(CONFIG_TCT_8X26_RIO6_TF)
if(1 == lcd_no_poweric) {
    ret = i2c_add_driver(&pwric_driver);
    if (ret) {
