...
{
	int i = 0, rc = 0;
	/*added by wangjie@tcl.com 2014/02/25 changing code for lcd white screen sometimes*/
#if defined(CONFIG_TCT_8X26_RIO5_CTCC)
       static int flag = 0;
#endif
       /*use to label the vdd has been enabled, which is only be enabled one time and not be disabled for white screen*/
       //printk("wangjie--%s: num_vreg=%d, enable=%d\n", __func__, num_vreg, enable);
	if (enable)
	{
#if defined(CONFIG_TCT_8X26_RIO5_CTCC)
		if(0 == flag)
		{
                        i = 0;
			rc = PTR_RET(in_vreg[0].vreg);
			if (rc) {
				DEV_ERR("%pS->%s: %s regulator error. rc=%d\n",
					__builtin_return_address(0), __func__,
					in_vreg[0].vreg_name, rc);
				goto vreg_set_opt_mode_fail;
			}
			if (in_vreg[0].pre_on_sleep)
				msleep(in_vreg[0].pre_on_sleep);
			rc = regulator_set_optimum_mode(in_vreg[0].vreg,
				in_vreg[0].enable_load);
			if (rc < 0) {
			...
			rc = regulator_enable(in_vreg[0].vreg);
			if (in_vreg[0].post_on_sleep)
				msleep(in_vreg[0].post_on_sleep);
			if (rc < 0) {
