	...
	if ((frame_rate !=
		...
		if (lanes > 0) {
			...
			((h_period * v_period *
			  frame_rate * bpp * 8)
			   / lanes);
			//add by yusen.ke.sz@tcl.com at 20140415 for cts test failed begin
			panel_info->clk_rate += 20000000;
			pr_err("%s: panel_info->clk_rate =%d\n", __func__, panel_info->clk_rate);	
			//add end
		} else {
