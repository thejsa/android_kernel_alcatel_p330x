...
/*[PLATFORM]-Add-BEGIN by TCTNB.YuBin, 2013/10/18,add charging enable/disable interface for fr540374*/
#ifdef CONFIG_MMI_TEST_CHARGING_CONTROL
#include <linux/miscdevice.h>
#include <linux/uaccess.h>
#include <linux/fs.h>
#endif
/*[PLATFORM]-Add-END by TCTNB.YuBin*/
/* [PLATFORM]-Mod-BEGIN by TCTNB.YuBin, 2013/11/28, refer to bug555496(miata), 562529(rio6),case01362220, Enable external ovp */
#if     (defined CONFIG_TCT_8X26_MIATA) || \
        (defined CONFIG_TCT_8X26_RIO6) || \
        (defined CONFIG_TCT_8X26_RIO6_TF) || \
        (defined CONFIG_TCT_8X26_RIO4G) || \
        (defined CONFIG_TCT_8X26_RIO4G_TF) || \
        (defined CONFIG_TCT_8X26_EOS) || \
        (defined CONFIG_TCT_8X26_RIO5_CTCC)
...
#define OVP_USB_WALL_THRESHOLD_MA	800 /* [PLATFORM]-Change-BEGIN by TCTSZ.cuiping.shi, for change chg current,2014/03/26*/
...
/*[PLATFORM]-Add-BEGIN by TCTNB.YuBin, 2013/11/21,add battery temperature protection for fr528245(miata), 551957(rio6,rio6_tf)*/
#if     (defined CONFIG_TCT_8X26_MIATA) || \
        (defined CONFIG_TCT_8X26_RIO6) || \
        (defined CONFIG_TCT_8X26_RIO6_TF) || \
        (defined CONFIG_TCT_8X26_RIO4G) || \
...
#endif
/* [PLATFORM]-Mod-END by TCTNB.YuBin */
/*[PLATFORM]-Add-BEGIN by TCTNB.YuBin, 2014/02/10, update batt temp*/
#if     (defined CONFIG_TCT_8X26_COMMON)
	struct delayed_work		batt_temp_update_work;
#endif
/*[PLATFORM]-Add-END by TCTNB.YuBin*/
...
/* [PLATFORM]-Mod-BEGIN by TCTNB.YuBin, 2013/11/28, refer to bug555496(miata), 562529(rio6),case01362220, Enable external ovp */
        ...
        (defined CONFIG_TCT_8X26_RIO5_CTCC)
static int
qpnp_chg_is_fast_chg_on(struct qpnp_chg_chip *chip)
{
	u8 fast_irq_rt_sts;
	int rc;
	...
	if (!chip->chgr_base)
		return 0;
	rc = qpnp_chg_read(chip, &fast_irq_rt_sts,
...
/* [PLATFORM]-Mod-BEGIN by TCTNB.YuBin, 2013/11/28, refer to bug555496(miata), 562529(rio6),case01362220, Enable external ovp */
	...
	fast_chg_on = qpnp_chg_is_fast_chg_on(chip);
		...
		if (fast_chg_on) {
			if (qpnp_chg_is_usb_chg_plugged_in(chip)) {
				...
				if ((ret.intval / 1000) > USB_WALL_THRESHOLD_MA) {
					...
					gpio_set_value(chip->ext_ovp_gpio, 1);
				}
			}
		} else {
			/* ToDo: Change EXT_OVP_EN (GPIO) to 0 */
			gpio_set_value(chip->ext_ovp_gpio, 0);
		}
...
static enum power_supply_property msm_batt_power_props[] = {
	...
	POWER_SUPPLY_PROP_CHARGE_VOLTAGE, // add by rongxiao.deng for get chg voltage
	POWER_SUPPLY_PROP_CHARGE_TEMP, // add by rongxiao.deng for get chg temp 
	POWER_SUPPLY_PROP_POWER_ON_VOLTAGE, // add by rongxiao.deng for get power on voltage 
...
/* [PLATFORM]-Mod-BEGIN by TCTNB.Yubin, 2013/11/28, refer to fr508845(miata), fr521336(rio6) */
#if     (defined CONFIG_TCT_8X26_MIATA) || \
        (defined CONFIG_TCT_8X26_RIO6) || \
        (defined CONFIG_TCT_8X26_RIO6_TF) || \
        (defined CONFIG_TCT_8X26_RIO4G) || \
        ...
        (defined CONFIG_TCT_8X26_EOS) || \
        (defined CONFIG_TCT_8X26_RIO5_CTCC)
	POWER_SUPPLY_PROP_CHARGER_IS_GOOD,
#endif
/* [PLATFORM]-Mod-END by TCTNB.YuBin */
...
get_prop_batt_health(struct qpnp_chg_chip *chip)
{
	u8 batt_health;
	int rc;
	pr_debug("chip->bat_health=%d \n",chip->bat_health);
/*[PLATFORM]-Add-BEGIN by TCTNB.YuBin, 2013/12/09, disable battery temperature check in NPI. fr498700*/
...
/*add by rongxiao.deng for eng mode*/
...
static int get_prop_power_on_voltage(struct qpnp_chg_chip *chip) // get power on voltage
{
        ...
        if (power_on_voltage ==3000)
        {
                ...
                power_on_voltage_smem = smem_alloc(SMEM_BATT_INFO, sizeof(uint32_t));
...
/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, PR-556220, 2013/11/19, adjust capacity from 100 to 99 if in charging */
/*
#ifdef CONFIG_TCT_8X26_COMMON
static int soc_pre = 100;
#endif
*/
...
/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, PR-556220, 2013/11/19, adjust capacity from 100 to 99 if in charging */
#ifdef CONFIG_TCT_8X26_COMMON
        if (soc == 100 && get_prop_batt_status(chip) == POWER_SUPPLY_STATUS_CHARGING) {
            pr_debug("still in charging, correct it to 99 !\n");
            soc = 99;
        }
        pr_debug("soc = %d, soc_pre = %d, usb_plugged_in = %d\n", soc, soc_pre, qpnp_chg_is_usb_chg_plugged_in(chip));
		// fix capacity jump to 100 from 99 after plug out usb
        if (soc_pre < soc && (!qpnp_chg_is_usb_chg_plugged_in(chip)))/*[PLATFORM]-Mod by TCTNB.YuBin, PR-561763*/
            soc = soc_pre;
        soc_pre = soc;
#endif
/* [PLATFORM]-Add-END by TCTNB.FLF */
...
/*[PLATFORM]-Add-BEGIN by TCTNB.YuBin, 2013/10/23, disable battery temperature check in NPI. fr498700*/
#ifdef FEATURE_TCTNB_MMITEST
	return 250;
#endif
/*[PLATFORM]-Add-BEGIN by TCTNB.YuBin*/
...
/*[PLATFORM]-Add-BEGIN by TCTNB.YuBin, 2013/12/13, stop charing for NPI in MINISW*/
#ifdef FEATURE_TCTNB_MMITEST
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		...
		break;
	case POWER_SUPPLY_PROP_POWER_ON_VOLTAGE: // add by rongxiao.deng for add some node
	...
	case POWER_SUPPLY_PROP_CHARGE_TEMP:
		...
		break;				// add by rongxiao.deng end
...
/* [PLATFORM]-Mod-BEGIN by TCTNB.Yubin, 2013/11/28, refer to fr508845(miata), fr521336(rio6) */
        ...
        (defined CONFIG_TCT_8X26_EOS) || \
        (defined CONFIG_TCT_8X26_RIO5_CTCC)
	case POWER_SUPPLY_PROP_CHARGER_IS_GOOD:
		if(qpnp_chg_usb_iusbmax_get(chip) < OVP_USB_WALL_THRESHOLD_MA)// modify by rongxiao.deng for bug634202
				val->intval = 0;
		else
				val->intval = 1;
		break;
...
/*[PLATFORM]-Add-BEGIN by TCTNB.YuBin, 2013/10/23, old code is not right, so modified it*/
...
qpnp_chg_adc_notification(enum qpnp_tm_state state, void *ctx)
{
	...
	pr_debug("qpnp_chg_adc_notification: temp = %d state = %s\n", temp,
			state == ADC_TM_WARM_STATE ? "over_highthr" : "over_lowthr");
	...
	if (state == ADC_TM_LOW_STATE)//higher then adc_param.high_temp
	{
		...
		{
			pr_err("error! temp:%d, batteryhealth:%d\n",temp,chip->bat_health);
		}
		else if(chip->bat_health == BATTERY_HEALTH_COLD)
		{
...
/*[PLATFORM]-Add-BEGIN by TCTNB.YuBin, 2014/02/10, update batt temp*/
#if     (defined CONFIG_TCT_8X26_COMMON)
#define BATT_TEMP_UPDATE_PERIOD_MS 20000
static void
qpnp_batt_temp_update_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct qpnp_chg_chip *chip = container_of(dwork,
				struct qpnp_chg_chip, batt_temp_update_work);
	static int pre_temp = 250;
	int temp = get_prop_batt_temp(chip);
	int voltage = get_prop_battery_voltage_now(chip);

	pr_debug("%s: pre_temp=%d, temp=%d; voltage=%d\n",__func__,  pre_temp, temp, voltage);

	if((pre_temp-temp>5) || (pre_temp-temp<-5) || (voltage < 3300000))
	{
		power_supply_changed(&chip->batt_psy);
		pre_temp = temp;
	}
	schedule_delayed_work(&chip->batt_temp_update_work,
		msecs_to_jiffies(BATT_TEMP_UPDATE_PERIOD_MS));
}
...
/* [PLATFORM]-Mod-BEGIN by TCTNB.YuBin, 2013/11/28, refer to bug555496(miata), 562529(rio6),case01362220, Enable external ovp */
        ...
        (defined CONFIG_TCT_8X26_EOS) || \
        (defined CONFIG_TCT_8X26_RIO5_CTCC)
	OF_PROP_READ(chip, ext_ovp_gpio, "ext-ovp-gpio", rc, 0);
#endif
/* [PLATFORM]-Mod-END by TCTNB.YuBin */
	if (rc)
...
static struct miscdevice charging_dev;
...
static long charging_dev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
        ...
        rc = qpnp_chg_charge_en(the_chip, !charging_disabled);
        rc |= qpnp_chg_force_run_on_batt(the_chip, charging_disabled);
        if(rc)
        {
                pr_err("%s failed\n", __func__);
                return -EFAULT;
...
/* [PLATFORM]-Mod-BEGIN by TCTNB.YuBin, 2013/11/28, refer to bug555496(miata), 562529(rio6),case01362220, Enable external ovp */
        ...
        (defined CONFIG_TCT_8X26_EOS) || \
        (defined CONFIG_TCT_8X26_RIO5_CTCC)
	if(gpio_request(chip->ext_ovp_gpio, "ext_ovp_control"))
	{
		pr_err("%s: request ext_ovp_gpio error\n",__func__);
	}
...
/*[PLATFORM]-Add-BEGIN by TCTNB.YuBin, 2014/02/10, update batt temp*/
#if     (defined CONFIG_TCT_8X26_COMMON)
	INIT_DELAYED_WORK(&chip->batt_temp_update_work, qpnp_batt_temp_update_work);
#endif
/*[PLATFORM]-Add-END by TCTNB.YuBin*/
...
/*[PLATFORM]-Add-BEGIN by TCTNB.YuBin, 2013/11/22, add battery id check function. fr545307, 580477*/
#ifndef FEATURE_TCTNB_MMITEST
#if     (defined CONFIG_TCT_8X26_MIATA) || \
        (defined CONFIG_TCT_8X26_RIO6)
	{
		struct qpnp_vadc_result batteryid;
		rc = qpnp_vadc_read(chip->vadc_dev, LR_MUX2_BAT_ID , &batteryid);
		if((!rc)&&(
		  /*SCUD*/((batteryid.physical>800000)&&(batteryid.physical<1000000)) /* [PLATFORM]-Change-BEGIN by TCTSZ.cuiping.shi for batt chg*/
		  /*MBELL*/ ||((batteryid.physical>200000)&&(batteryid.physical<400000))  )  ){
			chip->charging_disabled = false;
		}else{
			chip->charging_disabled = true;
			pr_err("battery id is not right,  rc=%d  ,    battery id(LR_MUX2_BAT_ID):  %lu  \n", rc, (unsigned long)(batteryid.physical));
		}
	}
#elif     (defined CONFIG_TCT_8X26_RIO6_TF)
	{
		struct qpnp_vadc_result batteryid;
		rc = qpnp_vadc_read(chip->vadc_dev, LR_MUX2_BAT_ID , &batteryid);
		if((!rc)&&(
		  /*SCUD*/((batteryid.physical>1000000)&&(batteryid.physical<1400000))  )  ){
			chip->charging_disabled = false;
		}else{
			chip->charging_disabled = true;
			pr_err("battery id is not right,  rc=%d  ,    battery id(LR_MUX2_BAT_ID):  %lu  \n", rc, (unsigned long)(batteryid.physical));
...
#elif     (defined CONFIG_TCT_8X26_RIO4G) || \
        (defined CONFIG_TCT_8X26_RIO4G_TF)
	{
		struct qpnp_vadc_result batteryid;
		rc = qpnp_vadc_read(chip->vadc_dev, LR_MUX2_BAT_ID , &batteryid);
		if((!rc)&&(
		  /*SCUD*/((batteryid.physical>1100000)&&(batteryid.physical<1300000))  ||
		  /*BYD*/((batteryid.physical>200000)&&(batteryid.physical<400000))  )  ){
			chip->charging_disabled = false;
		}else{
			chip->charging_disabled = true;
			pr_err("battery id is not right,  rc=%d  ,    battery id(LR_MUX2_BAT_ID):  %lu  \n", rc, (unsigned long)(batteryid.physical));
...
#endif
#endif //#ifndef FEATURE_TCTNB_MMITEST
/*[PLATFORM]-Add-BEGIN by TCTNB.YuBin*/
...
static int qpnp_chg_suspend(struct device *dev)
{
	...
	if (chip->bat_if_base) {
		rc = qpnp_chg_masked_write(chip,
			chip->bat_if_base + BAT_IF_VREF_BAT_THM_CTRL,
			VREF_BATT_THERM_FORCE_ON,
/* [PLATFORM]-Mod-BEGIN by TCTNB.YuBin, 2014/03/04, refer to case01468450, keep voltage always on */
			//old setting: VREF_BAT_THM_ENABLED_FSM, 1);
			VREF_BATT_THERM_FORCE_ON, 1);
/* [PLATFORM]-Mod-END by TCTNB.YuBin */
		if (rc)
