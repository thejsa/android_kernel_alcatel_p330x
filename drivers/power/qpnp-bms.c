...
static irqreturn_t bms_sw_cc_thr_irq_handler(int irq, void *_chip)
{
	...
	disable_bms_irq_nosync(&chip->sw_cc_thr_irq);/*[PLATFORM]-Add by TCTNB.YuBin, 2014/01/15, fix Qualcomm's bug, ref case01420985*/
	bms_stay_awake(&chip->soc_wake_source);
	schedule_work(&chip->recalc_work);
	return IRQ_HANDLED;
}
...
/* [PLATFORM]-Mod-BEGIN by TCTNB.YuBin, 2013/12/19, using battery data in c file */
	} else if (chip->batt_type == BATT_TCT_4V35_2000MAH_MIATA) {
		battery_id = read_battery_id(chip);
		if((battery_id>1100000)&&(battery_id<1300000))/*SCUD*/
			batt_data = &tct_4v35_2000mah_scud_miata;
		else if((battery_id>200000)&&(battery_id<400000))/*BYD*/
			batt_data = &tct_4v35_2000mah_byd_miata;
		else/*For MINI SW*/
			batt_data = &tct_4v35_2000mah_byd_miata;
	} else if (chip->batt_type == BATT_TCT_4V35_2000MAH_RIO4G) {
		battery_id = read_battery_id(chip);
		if((battery_id>1100000)&&(battery_id<1300000))/*SCUD*/
			batt_data = &tct_4v35_2000mah_scud_rio4g;
		else if((battery_id>200000)&&(battery_id<400000))/*BYD*/
			batt_data = &tct_4v35_2000mah_byd_rio4g;
		else/*For MINI SW*/
			batt_data = &tct_4v35_2000mah_byd_rio4g;
	} else if (chip->batt_type == BATT_TCT_4V35_2150MAH_EOS) {
		battery_id = read_battery_id(chip);
		if((battery_id>1000000)&&(battery_id<1400000))/*SCUD*/
			batt_data = &tct_4v35_2150mah_scud_eos;
		else if((battery_id>100000)&&(battery_id<500000))/*BYD*/
			batt_data = &tct_4v35_2150mah_byd_eos;
		else/*For MINI SW*/
			batt_data = &tct_4v35_2150mah_byd_eos;
	} else if (chip->batt_type == BATT_TCT_4V35_3400MAH_RIO6) {
		battery_id = read_battery_id(chip);
		if((battery_id>1000000)&&(battery_id<1400000))/*SCUD*/
			batt_data = &tct_4v35_3400mah_scud_rio6;
		else if((battery_id>100000)&&(battery_id<500000))/*BYD*/
			batt_data = &tct_4v35_3400mah_byd_rio6;
		else/*For MINI SW*/
			batt_data = &tct_4v35_3400mah_byd_rio6;
	} else if (chip->batt_type == BATT_TCT_4V35_2150MAH_RIO5) {
		batt_data = &tct_4v35_2150mah_byd_rio5;
...
assign_data:
	...
	if (batt_data->max_voltage_uv > 0 && dt_data) // [PLATFORM]-Add-BEGIN by TCTSZ.cuiping.shi, 2014/03/19
		chip->max_voltage_uv = batt_data->max_voltage_uv;
	if (batt_data->cutoff_uv > 0 && dt_data) // [PLATFORM]-Add-BEGIN by TCTSZ.cuiping.shi, 2014/03/19
		chip->v_cutoff_uv = batt_data->cutoff_uv;
	if (batt_data->iterm_ua > 0 && dt_data) // [PLATFORM]-Add-BEGIN by TCTSZ.cuiping.shi, 2014/03/19
		chip->chg_term_ua = batt_data->iterm_ua;

	// [PLATFORM]-Add-BEGIN by TCTSZ.cuiping.shi for print LOG, 2014/03/19
	pr_info("::line:%d, fcc_mah:%dmAh, max_voltage:%dmV, data_batt_id:%d\n",
		__LINE__,chip->fcc_mah,chip->max_voltage_uv/1000,batt_data->batt_id_kohm);
