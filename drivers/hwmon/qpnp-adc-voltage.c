...
struct qpnp_vadc_chip {
	...
	int bs_gpio_level;/* [PLATFORM]-ADD by TCTNB.YuBin, 2013/12/20, FR-552166, dev for board id */
};
...
/* [PLATFORM]-ADD-BEGIN by TCTNB.WJ, 2013/12/20, FR-534823 & 567374 & 575135, dev for HALL */
#if defined(CONFIG_TCT_8X26_RIO6)||defined(CONFIG_TCT_8X26_RIO6_TF)
static struct qpnp_vadc_chip *g_vadc = NULL;
#endif
...
/* [PLATFORM]-ADD-BEGIN by TCTNB.WJ, 2013/12/21, CR-534823 & 567374 & 575135, dev for HALL */
#if defined(CONFIG_TCT_8X26_RIO6)||defined(CONFIG_TCT_8X26_RIO6_TF)
int32_t qpnp_get_haptic_id(int64_t *haptic_id)
{
    int rc = -1;
    enum qpnp_vadc_channels channel = P_MUX8_1_1;
    struct qpnp_vadc_result result;
    *haptic_id = -1;

    if(g_vadc)
    {
        rc = qpnp_vadc_read(g_vadc, channel, &result);

        if (rc) {
            pr_err("VADC read haptic_id error with %d\n", rc);
            return rc;
        }

        *haptic_id = result.physical;
    }
    return rc;
}

EXPORT_SYMBOL(qpnp_get_haptic_id);
#endif
/* [PLATFORM]-ADD-END by TCTNB.WJ, 2013/12/21 */
			...
			struct device_attribute *devattr, char *buf)
{
	...
	struct qpnp_vadc_result result;
	int rc = -1;
/* [PLATFORM]-MOD-BEGIN by TCTNB.YuBin, 2013/12/20, FR-552166, dev for board id */
	if(TCT_BOARD_INFO == attr->index)
	{
		rc = qpnp_vadc_read(vadc, P_MUX7_1_3, &result);

		if(rc)
			return snprintf(buf, QPNP_ADC_HWMON_NAME_LENGTH,
				"MPP7:failed(%d),GPIO56:%d\n", rc, 1);
		else
			return snprintf(buf, QPNP_ADC_HWMON_NAME_LENGTH,
				"MPP7:%dmV,GPIO56:%d\n", (uint32_t)result.physical/1000, vadc->bs_gpio_level);
	}
...
static int __devinit qpnp_vadc_probe(struct spmi_device *spmi)
{
	...
	u8 fab_id = 0;
	int bs_gpio = -1;/* [PLATFORM]-ADD by TCTNB.YuBin, 2013/12/20, FR-552166, dev for board id */
...
/* [PLATFORM]-ADD-BEGIN by TCTNB.WJ, 2013/12/21, FR-534823 & 567374 & 575135, dev for HALL */
#if defined(CONFIG_TCT_8X26_RIO6)||defined(CONFIG_TCT_8X26_RIO6_TF)
    g_vadc = vadc;
#endif
/* [PLATFORM]-ADD-END by TCTNB.WJ */
...
/* [PLATFORM]-ADD-BEGIN by TCTNB.YuBin, 2013/12/20, FR-552166, dev for board id */
    vadc->bs_gpio_level = -1;
    bs_gpio = of_get_named_gpio(node, "qcom,bs-gpio", 0);
    if (gpio_is_valid(bs_gpio))
        vadc->bs_gpio_level = gpio_get_value(bs_gpio);
    else
        pr_err("tct_board_info: gpio%d is not valid!\n",bs_gpio);
/* [PLATFORM]-ADD-END by TCTNB.YuBin */
