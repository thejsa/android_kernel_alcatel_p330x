 ...
 *                  By Meta, 2013/06/08
 *      V1.9:
 *           modify auto update firmware compile error
 *             By NB.tcl XQJ and goodix FAE 2013/10/13
 */
...
/* [BUGFIX]-Add-BGEIN by TCTNB.WPL,2014/02/13, add by FAE, refer to bug557914 */
#define GTP_CLEAR_RAM  0  //[BUGFIX]-ADD-BEGIN by TCTNB.XQJ, 2014/2/17, bug 559289 ,not need this part,demand by FAE and test result
/* [BUGFIX]-Add-END by TCTNB.WPL,2014/02/13 */

#define GTP_ESD_CHECK_CIRCLE_MS	2000
u16     IC_VID=0;// [BUGFIX]-Add by TCTNB.XQJ, BUG-551802  2013/12/02,for unmount /data
extern const unsigned char header_fw_array[];// [BUGFIX]-Add by TCTNB.XQJ, BUG-551802 2013/12/02,for unmount /data
...
#endif
#endif
/*[BUGFIX]-ADD  by TCTNB.XQJ,12/31/2013,FR-575799,MIATA 2nd source .*/
#ifdef CONFIG_TOUCHSCREEN_FT5X06
 extern void  ft5x06_gpio_configure(void);
#endif
/*BUGFIX]-ADD  by TCTNB.XQJ*/
void gtp_reset_guitar(struct goodix_ts_data *ts, int ms);//[BUGFIX]-Add by TCTNB.XQJ, bug 	623533, 2014/03/19,For After TP Raw Data test,tp can't be used
...
u8 grp_cfg_version;
#ifdef TCT_TARGET_DBUNLOCK /*[BUGFIX]-Add by TCTNB.XQJ, FR-545213 2013/10/28,double click to unlock,dynamic setting */
static u8 dblick_switch=0;//0--close,1--open 
#undef  KEY_POWER
#define 	KEY_POWER  KEY_UNLOCK
#endif

/*[BUGFIX]-Add-BEGIN by TCTNB.(Ting Li), 2013/12/4 FR557207 , control touch in ipo poweroff .*/
#if TCT_TARGET_IPO
extern int ipo_touch_enable;
#endif
/*[BUGFIX]-Add-END by TCTNB.(Ting Li)*/
/*******************************************************
Function:
...
int gtp_i2c_read(struct i2c_client *client, u8 *buf, int len)
{
	...
	while (retries < 2) {/*[BUGFIX]-Add-by TCTNB.XQJ, FR-603186 2014/03/17,reduce retry times,for avoid triggering system about disable i2c int because of i2c error */
		ret = i2c_transfer(client->adapter, msgs, 2);
		if (ret == 2)
			break;
		retries++;
	}
	if (retries >= 2) { /*[BUGFIX]-Add-by TCTNB.XQJ, FR-603186 2014/03/17,reduce retry times,for avoid triggering system about disable i2c int because of i2c error */
#if GTP_SLIDE_WAKEUP
#ifdef TCT_TARGET_DBUNLOCK /*[BUGFIX]-Add- by TCTNB.XQJ, FR-545213 2013/10/28,double click to unlock,dynamic setting */
     if(dblick_switch==1)
#endif
...
void gtp_irq_disable(struct goodix_ts_data *ts)
{
	...
	if (!ts->irq_is_disabled) {
		...
		disable_irq_wake(ts->client->irq);/*[BUGFIX]-MOD-by TCTNB.XQJ, FR-545213 2013/10/28,for double click wake  */
...
void gtp_irq_enable(struct goodix_ts_data *ts)
{
	...
	spin_lock_irqsave(&ts->irq_lock, irqflags);
	if (ts->irq_is_disabled) {
		enable_irq_wake(ts->client->irq);/*[BUGFIX]-MOD-by TCTNB.XQJ, FR-545213 2013/10/28,for double click wake  */
	...
	if (DOZE_ENABLED == doze_status) {
		...
		if (ret > 0) {
			if (doze_buf[2] == 0xAA) {
				...
				input_sync(ts->input_dev);
				/* clear 0x814B */
                                /*[BUGFIX]-MOD-BEGIN  by TCTNB.XQJ,1/03/2013,PR-579182,not need clear it,for more safe*/
				//doze_buf[2] = 0x00;
...
/*[BUGFIX]-Add-BEGIN by TCTNB.(Ting Li), 2013/12/4 FR557207 , control touch in ipo poweroff .*/
        #if TCT_TARGET_IPO
        if (ipo_touch_enable == 0)
            return IRQ_HANDLED;
        #endif
/*[BUGFIX]-Add-END by TCTNB.(Ting Li)*/
	GTP_DEBUG_FUNC();
// [BUGFIX]-Add Bgein by TCTNB.XQJ, BUG-551802 2013/12/02,for remove error log,the first interrupt after init or wake up
...
/* [BUGFIX]-Add-BGEIN by TCTNB.WPL,2014/02/13, add by FAE, refer to bug557914 */
#if GTP_CLEAR_RAM
	u8 clear_assigned_ram[18]={0x80,0x30};
	int ret = -1;
#endif
/* [BUGFIX]-Add-END by TCTNB.WPL,2014/02/13 */
...
/* [BUGFIX]-Add-BGEIN by TCTNB.WPL,2014/02/13, add by FAE, refer to bug557914 */
	#if GTP_CLEAR_RAM
	gpio_direction_output(ts->pdata->irq_gpio, 1);
	memset(&clear_assigned_ram[2], 0, 16);
	ret = gtp_i2c_write(ts->client, clear_assigned_ram,18);
	if (ret>=0)
	{
		printk("Success to clear from 0x8030 to 0x803F to zero.\n");
	}
	else
	{
		printk("Failed to clear RAM from 0x8030 to 0x803F to zero.\n");
	}
	#endif
/* [BUGFIX]-Add-END by TCTNB.WPL,2014/02/13 */
...
#endif /*[BUGFIX]-Mod- by TCTNB.XQJ, FR-545213 2013/10/28,double click to unlock,dynamic setting */
/*******************************************************
Function:
	Enter sleep mode.
Input:
...
Output:
	...
	1: succeed, otherwise failed.
*******************************************************/
#if (GTP_DBL_CLK_WAKEUP==0) || defined(TCT_TARGET_DBUNLOCK)/*[BUGFIX]-Add- by TCTNB.XQJ, FR-545213 2013/10/28,double click to unlock,dynamic setting */
static s8 gtp_enter_sleep(struct goodix_ts_data  *ts)
{
	...
	ret = gpio_direction_output(ts->pdata->irq_gpio, 0); /*[BUGFIX]-Mod-by TCTNB.XQJ, PR-557434 2013/11/28, resetore */
	usleep(5000);
	while (retry++ < 5) {
		ret = gtp_i2c_write(ts->client, i2c_control_buf, 3);
		if (ret > 0) {
...
#else
	while (retry++ < 2) {/*[BUGFIX]-Mod-by TCTNB.XQJ, FR-603186 2014/03/17,old 10,reduce retry times,for avoid triggering system about disable i2c int because of i2c error */
pr_err("gtp_wakeup_sleep retry =%d",retry);
#if GTP_SLIDE_WAKEUP
#ifdef TCT_TARGET_DBUNLOCK /*[BUGFIX]-Add- by TCTNB.XQJ, FR-545213 2013/10/28,double click to unlock,dynamic setting */
  if(dblick_switch==1)
#endif
...
/* wakeup not by slide */
		if (DOZE_WAKEUP != doze_status)
		{
		    doze_status = DOZE_DISABLED;
		        gtp_irq_disable(ts);
/* [BUGFIX]-Add-BGEIN by TCTNB.WPL,2014/02/13, add by FAE, refer to bug557914 */
			msleep(300);//[BUGFIX]-ADD-BEGIN by TCTNB.XQJ, 2014/2/17, bug 559289 ,not need this part,demand by FAE and test result
/* [BUGFIX]-Add-END by TCTNB.WPL,2014/02/13 */
			gtp_reset_guitar(ts, 10);
			gtp_irq_enable(ts);
		}
...
#ifdef TCT_TARGET_DBUNLOCK /*[BUGFIX]-Add- by TCTNB.XQJ, FR-545213 2013/10/28,double click to unlock,dynamic setting */
/*[BUGFIX]-MOD Begin- by TCTNB.XQJ, FR-529784 2013/10/28,modify last commit about double click */
  else 
    {
         if (chip_gt9xxs == 0) {
/*[BUGFIX]-MOD Begin- by TCTNB.XQJ, PR-5861779, 2013/11/28, touch panel no response till 1 second late*/
            //  gtp_int_sync(ts, 25);
            //  msleep(200);
            gtp_irq_disable(ts);
            gtp_reset_guitar(ts, 10);
...
#if (!GTP_SLIDE_WAKEUP)/*[BUGFIX]-Add- by TCTNB.XQJ, FR-529784 2013/10/28,move the above*/
                   {
         if (chip_gt9xxs == 0) {
                     gtp_int_sync(ts, 25);
                     msleep(200);
...
#if 0  /*[BUGFIX]-Remove  by TCTNB.XQJ, FR-626357  2014/04/03,add 100um  cfg parameter manger ,now is same as 50um, */
...
/*[BUGFIX]-Mod  by TCTNB.XQJ, FR-626357  2014/04/03,add 100um  cfg parameter manger ,now is same as 50um, */
		if(sensor_id==1)
		{
		    config_data = ts->pdata->config_data_100um;
		    ts->config_data = ts->pdata->config_data_100um;
		    ts->gtp_cfg_len = ts->pdata->gtp_cfg_len_100um;
		}
/*[BUGFIX]-END by TCTNB.XQJ*/
		else
		{
...
int gtp_read_version(struct i2c_client *client, u16 *version)
{
	...
	if (version)
		*version = (buf[7] << 8) | buf[6];
        IC_VID=(buf[7] << 8) | buf[6];// [BUGFIX]-Add by TCTNB.XQJ, BUG-551802  2013/12/02,for unmount /data
	if (buf[5] == 0x00) {
...
static int gtp_i2c_test(struct i2c_client *client)
{
	...
	int retry = 2;/*[BUGFIX]-Mod-by TCTNB.XQJ, FR-603186 2014/03/17,reduce retry times,for avoid triggering system about disable i2c int because of i2c error */
	int ret = -EIO;
...
/*[BUGFIX]-Add by TCTNB.XQJ, 2014/2/19, bug 575661 reduce probe time for miata porject*/
#if defined CONFIG_TCT_8X26_MIATA
/*
  For miata ,check which tp,ft5x06 or gtp9xx,for saving mobile starting time,ft5x06
  first being running.
 first read ft5x06 i2c address,if read ok,so return 0,if read NOK,so continue read gt9xx tp,
 if read gt9xx i2c address ok,so return 1;if read gt9xx i2c address nok,so return 2
 ft5x06,i2c address is 0x38,gt9xx tp i2c address 0x14;
  0---ft5x06
  1---gt9xxx
  2---all nok,this is in ft5x06,if all nok,so continue probe.
*/
 int  tp_device_check(struct i2c_client *client) //
{
    u8 reg_value;
    u8 reg_addr = 0xA3;
    int ret=1;
    u8 reg_buf[3] = {0x81, 0x40};//for gt9xx read
    struct i2c_msg msgs[] = {
			{
				 .addr = 0x38,
				 .flags = 0,
    ...
    gpio_direction_output(17, 1);//for gt9xx tp special demand,
    ret = i2c_transfer(client->adapter, msgs, 2);
    if (ret < 0)//if not fa
    {
       pr_err("%s:ft i2c read,will try gt \n", __func__);

       msgs[0].flags = 0;
	msgs[0].addr = 0x14;
	msgs[0].len = 2;
	msgs[0].buf = &reg_buf[0];
	...
	msgs[1].buf = &reg_value;
       ret = i2c_transfer(client->adapter, msgs, 2);
       gpio_direction_input(17);//restore irq_gpio old seting;
	if (ret < 0)
        {
              pr_err( "%s: two  tp i2c  error,continue .\n",
                                  __func__);
               return 2;
        }
	 else
    ...
    gpio_direction_input(17);//restore irq_gpio input seting;
			...
			struct goodix_ts_platform_data *pdata)
{
 ...
 /*[BUGFIX]-Add Begin by TCTNB.XQJ, FR-626357  2014/04/03,add 100um  cfg parameter manger ,now is same as 50um, */
       prop = of_find_property(np, "goodix,cfg-data_100um", &pdata->gtp_cfg_len_100um);
	if (prop && prop->value) {
		pdata->config_data_100um= devm_kzalloc(dev,
			GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH, GFP_KERNEL);
		if (!pdata->config_data_100um) {
			dev_err(dev, "Not enough memory for panel config data\n");
			return -ENOMEM;
		}

		pdata->config_data_100um[0] = GTP_REG_CONFIG_DATA >> 8;
		pdata->config_data_100um[1] = GTP_REG_CONFIG_DATA & 0xff;
		memset(&pdata->config_data_100um[GTP_ADDR_LENGTH], 0,
					GTP_CONFIG_MAX_LENGTH);
		memcpy(&pdata->config_data_100um[GTP_ADDR_LENGTH],
				prop->value,GTP_CONFIG_MAX_LENGTH);
	} else {
		dev_err(dev,
			"Unable to get configure data 100um .default will  be used.\n");
		pdata->gtp_cfg_len_100um= 0;
	}
/*[BUGFIX]-Removed- end by TCTNB.XQJ*/
	return 0;
}

/*[BUGFIX]-Add- begin by TCTNB.XQJ, FR-545213 2013/10/28,double click to unlock,dynamic setting */
#ifdef TCT_TARGET_DBUNLOCK
static struct class *gt_dclick_class;/*[BUGFIX]-MOD by TCTNB.XQJ, FR-575799  2014/01/20,doublick compile problem */
static struct device *gt_dclick_dev;/*[BUGFIX]-MOD by TCTNB.XQJ, FR-575799  2014/01/20,double click compile problem*/
...
/*[BUGFIX]-Removed- begin by TCTNB.XQJ, PR-560980 2013/11/28,power comsuption,thiw work need't to been done */
#if 0
     if(dblick_switch==1)
     {
           /* wakeup not by slide */
...
#endif
 /*[BUGFIX]-END by TCTNB.XQJ*/
/*[BUGFIX]-Add-BEGIN by TCTNB.XQJ, 2014/1/9, bug 579792  for display  tp firmware version info*/
                                      ...
                                      struct device_attribute *attr, char *buf )
{
   ...
   u16 version_info;
   u8 sensor_id;
/*[BUGFIX]-ADD-BEGIN by TCTNB.XQJ, 2014/2/17, bug 559289  for display  tp config version info*/
   u8 tbuf[3]= { GTP_REG_CONFIG_DATA>> 8, GTP_REG_CONFIG_DATA & 0xff };
   ret = gtp_read_version(i2c_connect_client, &version_info);
   if (ret != 2) {
	   sprintf ( buf, "i2c  read error use  bakup , gt9xx TP value git9xx ic  version is  0x%x\n",IC_VID) ;
    }
    ret = gtp_i2c_read(i2c_connect_client, tbuf, 3);
    if (ret < 0) {
        pr_err( "GTP read config version  failed.\n");
    }
    ret = gtp_i2c_read_dbl_check(i2c_connect_client, GTP_REG_SENSOR_ID,
		&sensor_id, 1);
    if(ret<0){
     pr_err("Failed to get sensor_id!");
    }
 /*[BUGFIX]-END by TCTNB.XQJ*/
    return sprintf ( buf, "gt9xx TP ic version is  0x%x,CFG version=%d,sensor_id=%d\n",IC_VID,tbuf[2],sensor_id);//%x-->%d,for hardware demand
}
			   ...
			   const struct i2c_device_id *id)
{
	...
	u16 version_info;
	int ret;
#if GTP_AUTO_UPDATE //[PLATFORM]-Add  by TCTNB.XQJ,12/16/2013,FR-564638,for compile
      u16 default_FW_VS;//[BUGFIX]- by TCTNB.XQJ, 2013/12/02,for unmount /data
#endif
...
//#endif
	...
	gtp_reset_guitar(ts, 20);
/*[BUGFIX]-Mod by TCTNB.XQJ, 2014/2/19, bug 575661 reduce probe time for miata porject*/
#if defined CONFIG_TCT_8X26_MIATA
       if(0==tp_device_check(client))//
       {
...
// [BUGFIX]-END  by TCTNB.XQJ
#if GTP_AUTO_UPDATE
//[BUGFIX]-Add  begin by TCTNB.XQJ, 2013/12/02,for unmount /data
...
// [BUGFIX]-END  by TCTNB.XQJ
    ...
    firm_ver_attr_create();/*[BUGFIX]-Add- by TCTNB.XQJ, 2014/1/9, bug 579792  for display  tp firmware version info*/
#ifdef TCT_TARGET_DBUNLOCK /*[BUGFIX]-Add- begin by TCTNB.XQJ, FR-545213 2013/10/28,double click to unlock,dynamic setting */
       DBclick_attr_create();
#endif
	init_done = true;
	return 0;
...
static void goodix_ts_suspend(struct goodix_ts_data *ts)
{
  ...
  else
  {
      if (ts->use_irq)
		...
		hrtimer_cancel(&ts->timer);
/*[BUGFIX]-Removed -by TCTNB.XQJ, FR-603186 2014/03/17,sugestiong from supplier, this part have doned in interrupt work_f */
	//for (i = 0; i < GTP_MAX_TOUCH; i++)
	//	gtp_touch_up(ts, i);
...
// [BUGFIX]-Add End by TCTNB.XQJ
	 ...
	 */
         ...
         init_done =false;// [BUGFIX]-Add by TCTNB.XQJ, BUG-551802  2013/12/02,for remove error i2c log in init or wakeup process
...
static void goodix_ts_resume(struct goodix_ts_data *ts)
{
 ...
 /*[BUGFIX]-END by TCTNB.XQJ*/
    ...
    init_done=true;//[BUGFIX]-Add by TCTNB.XQJ, BUG-551802,2013/12/02,for remove error i2c log in init or wakeup process
