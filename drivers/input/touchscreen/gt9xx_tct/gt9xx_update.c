 ...
 *                          By Meta, 2013/03/11
 *      V1.7:
 *            modify auto update compile error ,can auto update firmeare truely
 *              BY NB.TCL.XQJ,2013/10/14
 */
...
static u8 gup_enter_update_judge(st_fw_head *fw_head)
{
	...
	if (!memcmp(fw_head->hw_info, update_msg.ic_fw_msg.hw_info,
		...
		(!memcmp(fw_head->pid, "91XX", 4))) {
			...
			/* The third condition */
/*[BUGFIX]-MOD-BEGIN  by TCTNB.XQJ,1/03/2013,PR-578520 upgrade only firmware file version >now tp version*/
			if (fw_head->vid > update_msg.ic_fw_msg.vid) {// [BUGFIX]-Add  by TCTNB.XQJ, BUG-551802,2013/12/02,for unmount /data
		...
		*4 + FW_DSP_ISP_LENGTH+FW_DSP_LENGTH + FW_BOOT_LENGTH)) {
		...
		for (i = 0; i < GUP_SEARCH_FILE_TIMES; i++) {
			...
			if (!(got_file_flag&0x0F)) {
				update_msg.file =filp_open(search_update_path,//[PLATFORM]-Add  by TCTNB.XQJ,12/16/2013,FR-564638,for compile
						O_RDONLY, 0);
				if (!IS_ERR(update_msg.file)) {
