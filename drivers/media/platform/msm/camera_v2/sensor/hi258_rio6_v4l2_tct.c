 ...
 * --------------------------------------------------------------------------------------
 * 13/11/27      Wang Penglei       Add driver for RIO6-Global's 1st sub CAM
 * 13/12/30      Wang Penglei       Modify sequence for RIO6-Global's 1st sub CAM
 * 14/03/27      Hu Jin		 sync with miata
 * --------------------------------------------------------------------------------------
*/
#include "msm_sensor.h"
#include "msm_cci.h"
#include "msm_camera_io_util.h"
#define HI258_RIO6_SENSOR_NAME "hi258_rio6"
...
DEFINE_MSM_MUTEX(hi258_rio6_mut);
static struct msm_sensor_ctrl_t hi258_rio6_s_ctrl;

static struct msm_sensor_power_setting hi258_rio6_power_setting[] = {
	{
...
// preview setting
static struct msm_camera_i2c_reg_conf hi258_rio6_preview_settings[] = {
//{0x03, 0x00},
//{0x01, 0x01}, //HI258_RIO6_Sleep_Mode
...
{0x30, 0x06},
{0x31, 0x40},
//{0x01, 0x00}, //HI258_RIO6_Sleep_Mode
...
// snapshot setting
static struct msm_camera_i2c_reg_conf hi258_rio6_snapshot_settings[] = {
/* 2M FULL Mode */
//{0x03,0x00},
//{0x01, 0x01},  // HI258_RIO6_Sleep_Mode
...
static struct msm_camera_i2c_reg_conf hi258_rio6_start_settings[] = {
	{0x03, 0x00},
	{0x01, 0x00},
};

static struct msm_camera_i2c_reg_conf hi258_rio6_stop_settings[] = {
	{0x03, 0x00},
	{0x01, 0x01},
};
...
static struct msm_camera_i2c_reg_conf hi258_rio6_recommend_settings[] = {
{0x01, 0x01}, //sleep on
{0x01, 0x03}, //sleep off
{0x01, 0x01}, //sleep on
// PAGE 20
...
static struct v4l2_subdev_info hi258_rio6_subdev_info[] = {
	{
...
static const struct i2c_device_id hi258_rio6_i2c_id[] = {
	{HI258_RIO6_SENSOR_NAME, (kernel_ulong_t)&hi258_rio6_s_ctrl},
	{ }
};

static int32_t msm_hi258_rio6_i2c_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	return msm_sensor_i2c_probe(client, id, &hi258_rio6_s_ctrl);
}

static struct i2c_driver hi258_rio6_i2c_driver = {
	.id_table = hi258_rio6_i2c_id,
	.probe  = msm_hi258_rio6_i2c_probe,
	.driver = {
		.name = HI258_RIO6_SENSOR_NAME,
	},
};

static struct msm_camera_i2c_client hi258_rio6_sensor_i2c_client = {
	.addr_type = MSM_CAMERA_I2C_BYTE_ADDR,
};

static const struct of_device_id hi258_rio6_dt_match[] = {
	{.compatible = "qcom,hi258_rio6", .data = &hi258_rio6_s_ctrl},
	{}
};

MODULE_DEVICE_TABLE(of, hi258_rio6_dt_match);

static struct platform_driver hi258_rio6_platform_driver = {
	.driver = {
		.name = "qcom,hi258_rio6",
		.owner = THIS_MODULE,
		.of_match_table = hi258_rio6_dt_match,
	},
};

static int32_t hi258_rio6_platform_probe(struct platform_device *pdev)
{
	int32_t rc;
	const struct of_device_id *match;

	match = of_match_device(hi258_rio6_dt_match, &pdev->dev);
	rc = msm_sensor_platform_probe(pdev, match->data);
	return rc;
}

static int __init hi258_rio6_init_module(void)
{
	int32_t rc;
	pr_info("%s:%d\n", __func__, __LINE__);
	
	rc = platform_driver_probe(&hi258_rio6_platform_driver,
		hi258_rio6_platform_probe);
	if (!rc)
		return rc;
	pr_err("%s:%d rc %d\n", __func__, __LINE__, rc);
	return i2c_add_driver(&hi258_rio6_i2c_driver);
}

static void __exit hi258_rio6_exit_module(void)
{
	pr_info("%s:%d\n", __func__, __LINE__);
	if (hi258_rio6_s_ctrl.pdev) {
		msm_sensor_free_sensor_data(&hi258_rio6_s_ctrl);
		platform_driver_unregister(&hi258_rio6_platform_driver);
	} else
		i2c_del_driver(&hi258_rio6_i2c_driver);
	return;
}

int32_t hi258_rio6_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	int32_t rc = 0;
	uint16_t chipid = 0;
...
int32_t hi258_rio6_sensor_config(struct msm_sensor_ctrl_t *s_ctrl,
	void __user *argp)
{
	...
	case CFG_SET_INIT_SETTING:
		...
		rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->
		    i2c_write_conf_tbl(
		    s_ctrl->sensor_i2c_client, hi258_rio6_recommend_settings,
		    ARRAY_SIZE(hi258_rio6_recommend_settings),
	...
	case CFG_SET_RESOLUTION: {
		...
		rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->
			i2c_write_conf_tbl(
			s_ctrl->sensor_i2c_client, hi258_rio6_preview_settings,
			ARRAY_SIZE(hi258_rio6_preview_settings),
			MSM_CAMERA_I2C_BYTE_DATA);
		else if (val == 0) // val==0, snapshot setting
		rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->
			i2c_write_conf_tbl(
			s_ctrl->sensor_i2c_client, hi258_rio6_snapshot_settings,
			ARRAY_SIZE(hi258_rio6_snapshot_settings),
			MSM_CAMERA_I2C_BYTE_DATA);
		break;
		}
...
/*[BUGFIX]-Mod-BEGIN by TCTNB.WPL, bug564473, 2013/11/30 */
		rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->
			i2c_write_conf_tbl(
			s_ctrl->sensor_i2c_client, hi258_rio6_stop_settings,
			ARRAY_SIZE(hi258_rio6_stop_settings),
			MSM_CAMERA_I2C_BYTE_DATA);
/*[BUGFIX]-Mod-END by TCTNB.WPL */
		break;
...
/*[BUGFIX]-Mod-BEGIN by TCTNB.WPL, bug564473, 2013/11/30 */
		rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->
			i2c_write_conf_tbl(
			s_ctrl->sensor_i2c_client, hi258_rio6_start_settings,
			ARRAY_SIZE(hi258_rio6_start_settings),
			MSM_CAMERA_I2C_BYTE_DATA);
/*[BUGFIX]-Mod-END by TCTNB.WPL */
		break;
...
static struct msm_sensor_fn_t hi258_rio6_sensor_func_tbl = {
	.sensor_config = hi258_rio6_sensor_config,
	.sensor_power_up = msm_sensor_power_up,
	.sensor_power_down = msm_sensor_power_down,
	.sensor_match_id = hi258_rio6_sensor_match_id,
};

static struct msm_sensor_ctrl_t hi258_rio6_s_ctrl = {
	.sensor_i2c_client = &hi258_rio6_sensor_i2c_client,
	.power_setting_array.power_setting = hi258_rio6_power_setting,
	.power_setting_array.size = ARRAY_SIZE(hi258_rio6_power_setting),
	.msm_sensor_mutex = &hi258_rio6_mut,
	.sensor_v4l2_subdev_info = hi258_rio6_subdev_info,
	.sensor_v4l2_subdev_info_size = ARRAY_SIZE(hi258_rio6_subdev_info),
	.func_tbl = &hi258_rio6_sensor_func_tbl,
};

module_init(hi258_rio6_init_module);
module_exit(hi258_rio6_exit_module);
