 ...
 * 13/08/28      Hu Jin       Add OTP support
 * 13/10/29      Hu Jin       Revert the power control
 * 13/11/07      Hu Jin       add s5k4h5yc for EOS
 * 13/12/11      Hu Jin       all i2c
...
/* [PLATFORM]-Add-BEGIN by TCTNB.HJ, 2013/08/23, OTP dev */
#ifdef CONFIG_TCT_8X26_COMMON
#define TCT_SENSOR_OTP_OPEN 1
#if TCT_SENSOR_OTP_OPEN
#include "s5k4h5yc_miata_v4l2_tct_OTP.h"
#include "s5k4h5yc_eos_v4l2_tct_OTP.h"
#include "s5k4h5yc_rio6_v4l2_tct_OTP.h"
#include "ov5648ff_rio4g_v4l2_tct_OTP.h"
#include "ov5648_rio4g_v4l2_tct_OTP.h"
#include "ov5648_rio4g_2nd_v4l2_tct_OTP.h"
/*ov5648_miata was canceled, but used for rio5_ctcc 1st main cam*/
#include "ov5648_miata_v4l2_tct_OTP.h"
#include "ov8858_eos_v4l2_tct_OTP.h"
#include "s5k5e2_rio5_ctcc_v4l2_tct_OTP.h"
#endif
#endif
...
#ifdef CONFIG_TCT_8X26_COMMON
static int TCT_request_CAM_gpio(unsigned CAM_gpio)
{
    char name[20];

	sprintf(name, "CAM_gpio_%d", CAM_gpio);
    if (gpio_is_valid(CAM_gpio)) {
        //printk("CAM_ID_PIN name:%s\n", name);
		gpio_request(CAM_gpio, name);
        gpio_tlmm_config(GPIO_CFG(CAM_gpio, 0,
			GPIO_CFG_INPUT, GPIO_CFG_NO_PULL,
			GPIO_CFG_2MA), GPIO_CFG_ENABLE);
    }else{
		printk("CAM_ID_PIN ERROR %s\n", __func__);
	}
    return 0;
}
static int TCT_GET_MODULE_ID(struct msm_sensor_ctrl_t *s_ctrl)
{
    uint8_t module_id = 0;
	if (!s_ctrl || !s_ctrl->sensor_i2c_client || !s_ctrl->sensor_i2c_client->i2c_func_tbl|| !s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write){
		printk("CAM some error\n");
        return 0;
    }
	s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(s_ctrl->sensor_i2c_client,
									0x0100, 0x01, MSM_CAMERA_I2C_BYTE_DATA);
    mdelay(10);
    if (!strcmp(s_ctrl->sensordata->sensor_name, "ov5648_rio4g")){
        printk("OTP: ov5648_rio4g\n");
        ov5648_rio4g_1st_update_otp(s_ctrl->sensor_i2c_client, &module_id);
	}else if(!strcmp(s_ctrl->sensordata->sensor_name, "ov5648_rio4g_2nd")){
        printk("OTP: ov5648_rio4g_2nd\n");
        ov5648_rio4g_2nd_1st_update_otp(s_ctrl->sensor_i2c_client, &module_id);
    }
    s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(s_ctrl->sensor_i2c_client,
									0x0100, 0x00, MSM_CAMERA_I2C_BYTE_DATA);
	return module_id;
}
static int TCT_GET_CAM_ID_PIN(struct msm_sensor_ctrl_t *s_ctrl)
{
    int rc = 0;
    uint8_t module_id = 0;
    int CAM_ID_PIN_VALUE = 0;
	unsigned CAM_ID_PIN = 0;
	unsigned CAM_ID_PIN_CNT = 0;

    CAM_ID_PIN_CNT = of_gpio_named_count(s_ctrl->pdev->dev.of_node, "gpio_CAM_ID");
    //printk("CAM_ID_PIN_CNT = [%d]\n", CAM_ID_PIN_CNT);
	if (CAM_ID_PIN_CNT == 1){//for used CAM
		CAM_ID_PIN = of_get_named_gpio(s_ctrl->pdev->dev.of_node, "gpio_CAM_ID", 0);
		TCT_request_CAM_gpio(CAM_ID_PIN);
        CAM_ID_PIN_VALUE = gpio_get_value(CAM_ID_PIN);
        printk("CAM_ID_PIN_VALUE = [%d]\n", CAM_ID_PIN_VALUE);
        if (!strcmp(s_ctrl->sensordata->sensor_name, "ov5648ff_rio4g")){
            if (CAM_ID_PIN_VALUE == 1){
                //printk("CAM_ID_PIN: ov5648ff_rio4g succeed\n");
            }else{
                rc = -ENODEV;
            }
        }else if (!strcmp(s_ctrl->sensordata->sensor_name, "ov5648_rio4g")){
            if (CAM_ID_PIN_VALUE == 0){
                module_id = TCT_GET_MODULE_ID(s_ctrl);
                if(module_id == 0x06){
					rc = 0;
                }else{
					if(!module_id)
                        printk("it should make some mistake %d\n", __LINE__);
					rc = -ENODEV;
                }
            }else{
                rc = -ENODEV;
        ...
        }else{//for un-used CAM
        }
        gpio_free(CAM_ID_PIN);
    }else if(CAM_ID_PIN_CNT > 1){//but also return 0 here.
		printk("CAM_ID_PIN %s it should be make some err\n", __func__);
    }else{//for un-used CAM
    }
	return rc;
}
...
/* [PLATFORM]-Mod-BEGIN by TCTNB.HJ, 2014/01/09*/
#ifdef CONFIG_TCT_8X26_COMMON
    struct msm_camera_cci_client *cci_client;
	if ((!strcmp(s_ctrl->sensordata->sensor_name, "s5k4h5yc_miata"))||
        (!strcmp(s_ctrl->sensordata->sensor_name, "s5k4h5yc_eos"))||
        (!strcmp(s_ctrl->sensordata->sensor_name, "s5k4h5yc_rio6"))){
        ...
        if(rc < 0){
            if(s_ctrl->sensordata->slave_info->sensor_slave_addr == 0x20){
				s_ctrl->sensordata->slave_info->sensor_slave_addr = 0x30;
				cci_client->sid = s_ctrl->sensordata->slave_info->sensor_slave_addr >> 1;
            }else if(s_ctrl->sensordata->slave_info->sensor_slave_addr == 0x30){
				s_ctrl->sensordata->slave_info->sensor_slave_addr = 0x20;
				cci_client->sid = s_ctrl->sensordata->slave_info->sensor_slave_addr >> 1;
            }else{
				printk("MATCH: it should be make some mistakes: %d\n", __LINE__);
            }
			rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(
                s_ctrl->sensor_i2c_client,
                s_ctrl->sensordata->slave_info->sensor_id_reg_addr,
...
#endif
/* [PLATFORM]-Mod-END by TCTNB.HJ, 2014/01/09*/
	if (rc < 0) {
...
/* [PLATFORM]-Add-BEGIN by TCTNB.HJ, 2013/12/12*/
#ifdef CONFIG_TCT_8X26_COMMON
	if (TCT_GET_CAM_ID_PIN(s_ctrl)) {
        printk("IDPIN some error\n");
		return -ENODEV;
    }
#endif
/* [PLATFORM]-Add-END by TCTNB.HJ, 2013/12/12*/
	return rc;
}
...
{
    mutex_lock(s_ctrl->msm_sensor_mutex);
    if (!strcmp(s_ctrl->sensordata->sensor_name, "ov5648_miata")){
		printk("OTP: ov5648_miata\n");
        ov5648_miata_1st_update_otp(s_ctrl->sensor_i2c_client);
    }else if (!strcmp(s_ctrl->sensordata->sensor_name, "ov5648_rio5_ctcc")){
		printk("OTP: ov5648_rio5_ctcc\n");
	/*same with miata*/
        ov5648_miata_1st_update_otp(s_ctrl->sensor_i2c_client);
    }else if (!strcmp(s_ctrl->sensordata->sensor_name, "ov5648ff_rio4g")){
		printk("OTP: ov5648ff_rio4g\n");
        ov5648ff_rio4g_1st_update_otp(s_ctrl->sensor_i2c_client);
    }else if (!strcmp(s_ctrl->sensordata->sensor_name, "ov5648_rio4g")){
		printk("OTP: ov5648_rio4g\n");
        ov5648_rio4g_1st_update_otp(s_ctrl->sensor_i2c_client, NULL);
    }else if (!strcmp(s_ctrl->sensordata->sensor_name, "ov5648_rio4g_2nd")){
		printk("OTP: ov5648_rio4g_2nd\n");
        ov5648_rio4g_2nd_1st_update_otp(s_ctrl->sensor_i2c_client, NULL);
    }else if (!strcmp(s_ctrl->sensordata->sensor_name, "s5k4h5yc_miata")){
		printk("OTP: s5k4h5yc_miata\n");
        if(s_ctrl->sensordata->slave_info->sensor_slave_addr == 0x20){
			printk("old i2c\n");
        }else if(s_ctrl->sensordata->slave_info->sensor_slave_addr == 0x30){
			printk("new i2c\n");
        }else{
			printk("it should be some mistakes %d\n", __LINE__);
        }
        S5K4H5YC_MIATA_TCT_OTP_calibration(s_ctrl->sensor_i2c_client);
    }else if (!strcmp(s_ctrl->sensordata->sensor_name, "s5k4h5yc_eos")){
		printk("OTP: s5k4h5yc_eos\n");
        if(s_ctrl->sensordata->slave_info->sensor_slave_addr == 0x20){
			printk("old i2c\n");
        }else if(s_ctrl->sensordata->slave_info->sensor_slave_addr == 0x30){
			printk("new i2c\n");
        ...
        S5K4H5YC_EOS_TCT_OTP_calibration(s_ctrl->sensor_i2c_client);
    }else if (!strcmp(s_ctrl->sensordata->sensor_name, "s5k4h5yc_rio6")){
		printk("OTP: s5k4h5yc_rio6\n");
        if(s_ctrl->sensordata->slave_info->sensor_slave_addr == 0x20){
			printk("old i2c\n");
        }else if(s_ctrl->sensordata->slave_info->sensor_slave_addr == 0x30){
			printk("new i2c\n");
        ...
        S5K4H5YC_RIO6_TCT_OTP_calibration(s_ctrl->sensor_i2c_client);
    }else if (!strcmp(s_ctrl->sensordata->sensor_name, "ov8858_eos")||
		!strcmp(s_ctrl->sensordata->sensor_name, "ov8858_miata")||
		!strcmp(s_ctrl->sensordata->sensor_name, "ov8858_rio6"))
    {
		printk("OTP: ov8858_e_m_r\n");
        ov8858_eos_TCT_OTP_calibration(s_ctrl->sensor_i2c_client);
    }else if (!strcmp(s_ctrl->sensordata->sensor_name, "s5k5e2_rio5_ctcc")){
		printk("OTP: s5k5e2_rio5_ctcc\n");
        S5K5E2_RIO5_CTCC_TCT_OTP_calibration(s_ctrl->sensor_i2c_client);
    }else{
