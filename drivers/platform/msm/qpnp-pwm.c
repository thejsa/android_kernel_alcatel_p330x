...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2013/11/28, Set 9 bit mode for wled:backlight */
#ifdef CONFIG_TCT_8X26_COMMON
		if (pwm_config->channel_id == 5) {
			printk("Use 9 bit for wled:backlight(channel id: 5)\n");
			pwm->pwm_config.period.pwm_size = 9;
			pwm->pwm_config.period.clk = 2;
			pwm->pwm_config.period.pre_div = 0;
			pwm->pwm_config.period.pre_div_exp = 0;
		} else {
