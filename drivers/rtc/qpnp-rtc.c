...
/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, FR-467329, 2013/09/29,  power off alarm */
#ifdef CONFIG_TCT_8X26_COMMON
    pr_debug("--pwroffalarm-- %s: enabled = %d(%s)\n", __func__, enabled, current->comm);
#endif
/* [PLATFORM]-Add-END by TCTNB.FLF */
...
/* [PLATFORM]-Add-END by TCTNB.FLF */
	...
	if (rc)
		dev_err(rtc_dd->rtc_dev,
				"Write to ALARM control reg failed\n");
/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, FR-467329, 2013/10/24,  power off alarm report key */
#ifdef CONFIG_TCT_8X26_COMMON
    printk("--pwroffalarm-- %s: input_report_key>>>>>>>>>>\n", __func__);
...
/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, FR-467329, 2013/10/24,  power off alarm report key for pwr off charge */
#ifdef CONFIG_TCT_8X26_COMMON
	rtc_dd->ip_dev = input_allocate_device();
	if (!rtc_dd->ip_dev) {
		pr_err("--pwroffalarm-- %s: could not allocate rtc alarm  input device\n", __func__);
...
/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, FR-467329, 2013/09/29,  power off alarm */
#if defined (CONFIG_TCT_8X26_COMMON) && !defined (FEATURE_TCTNB_MMITEST)    // no power off alarm in Mini version
    printk("--pwroffalarm-- %s: rtc_alarm_powerup = %d, power_on_alarm = %ld\n", __func__, rtc_alarm_powerup, power_on_alarm);
	if (!rtc_alarm_powerup && !power_on_alarm)
#else
