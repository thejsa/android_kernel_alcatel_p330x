...
/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, FR-467329, 2013/10/31,  power off alarm */
#ifdef CONFIG_TCT_8X26_COMMON
#define ALARM_DELTA 2
#else
...
/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, FR-467329, 2013/09/29,  power off alarm */
#ifdef CONFIG_TCT_8X26_COMMON
long power_on_alarm;
#else
...
/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, FR-467329, 2013/09/29,  power off alarm */
#ifdef CONFIG_TCT_8X26_COMMON
int pwroff_set_rtc_alarm(struct timespec new_time)
{
	int ret=0;
	...
	if (!alarm_rtc_dev) {
        printk("--pwroffalarm-- %s: alarm_rtc_dev no that device\n", __func__);
		return -ENODEV;
	}
	...
	printk("--pwroffalarm-- %s: new_time.tv_sec = %ld, rtc_current_time = %lu\n", __func__, new_time.tv_sec, rtc_current_time);
	new_time.tv_sec=new_time.tv_sec+rtc_current_time;
    rtc_time_to_tm(new_time.tv_sec, &rtc_alarm.time);//test

    pr_debug("--pwroffalarm-- %s: set rtc %ld %ld - rtc %02d:%02d:%02d %02d/%02d/%04d\n", __func__,
	...
	if (ret < 0)
		printk("--pwroffalarm-- %s: Failed to set power off alarm\n", __func__);
...
/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, FR-467329, 2013/10/11,  power off alarm */
#ifdef CONFIG_TCT_8X26_COMMON
static void alarm_shutdown(struct platform_device *dev)
{
    ...
    struct rtc_wkalrm alarm;
    unsigned long flags;
    long rtc_secs, alarm_time = 0;
    int rc;

    //printk("--pwroffalarm-- %s: power_on_alarm = %ld\n", __func__, power_on_alarm);
    spin_lock_irqsave(&alarm_slock, flags);
    ...
    pr_debug("--pwroffalarm-- %s: read rtc_current_rtc_time - rtc %02d:%02d:%02d %02d/%02d/%04d\n", __func__,
            rtc_time.tm_hour, rtc_time.tm_min,
            rtc_time.tm_sec, rtc_time.tm_mon + 1,
            rtc_time.tm_mday, rtc_time.tm_year + 1900);

    rtc_tm_to_time(&rtc_time, &rtc_secs);
    rtc_secs += power_on_alarm;
     ...
     * expiration.
     */
    if (power_on_alarm > ALARM_DELTA)
        rtc_secs -= ALARM_DELTA;
    //printk("--pwroffalarm-- %s: set rtc_secs = %ld\n", __func__, rtc_secs);
    rtc_time_to_tm(rtc_secs, &alarm.time);

    pr_debug("--pwroffalarm-- %s: set rtc - rtc h/m/s:m/d/y %02d:%02d:%02d %02d/%02d/%04d\n", __func__,
...
/*[BUGFIX]-DEL-BEGIN,fangyou.wang,2014/02/28, FR-597891, qcom patch*/
  //  spin_unlock_irqrestore(&alarm_slock, flags);
/*[BUGFIX]-DEL-END,fangyou.wang,2014/02/28, FR-597891, qcom patch*/
    return;

disable_alarm:
    //printk("%s: disable_alarm\n", __func__);
