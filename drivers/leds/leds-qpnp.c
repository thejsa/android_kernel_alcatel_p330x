...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2013/8/13, Add lcd backlight driver for IC KTD2599*/
#ifdef CONFIG_TCT_8X26_RIOL
#include <linux/delay.h>
#include <linux/gpio.h>
#include <linux/qpnp/qpnp-adc.h>

#define GPIO_LCD_BACKLIGHT_EN 52
#define KTD2599_DEVICE_ADDRESS 0x72
...
/*[PLATFORM]-Add-BEGIN by TCTNB.YJ, 2013/12/20, PR-561125, PR-562151 */
#if defined(CONFIG_TCT_8X26_RIO4G) || defined(CONFIG_TCT_8X26_RIO5_CTCC) || defined(CONFIG_TCT_8X26_RIO4G_TF)
/* For Rio4G only, as it has flash led with max current limitted to 120mA for flash mode, 60mA max for Torch mode */
#define FLASH_MAX_LEVEL			0x08
#define TORCH_MAX_LEVEL			0x03
#else
...
/*[PLATFORM]-Add-BEGIN by TCTNB.(WJ), FR-551602, 2013/11/12*/
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2013/8/13, Add lcd backlight driver for IC KTD2599*/
#ifdef CONFIG_TCT_8X26_RIOL
static void ktd2599_init(void)
{
	int err;
	err = gpio_request(GPIO_LCD_BACKLIGHT_EN, "lcd_backlight_gpio");
	if (err) {
		printk("bl: gpio request failed\n");
	}

	err = gpio_tlmm_config(GPIO_CFG(GPIO_LCD_BACKLIGHT_EN, 0, GPIO_CFG_OUTPUT,
								  GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (err) {
		printk("bl: gpio config failed\n");
	}

	err = gpio_direction_output(GPIO_LCD_BACKLIGHT_EN, 0);
	if (err) {
		printk("bl: set_direction for gpio failed\n");
	}
}

static void ktd2599_remove(void)
{
	gpio_free(GPIO_LCD_BACKLIGHT_EN);
}

static void ktd2599_set_mode(void)
{
...
static void ktd2599_shut_down(void)
{
	gpio_set_value(GPIO_LCD_BACKLIGHT_EN, 0);
	mdelay(3);
}

static void ktd2599_send_low(void)
{
	gpio_set_value(GPIO_LCD_BACKLIGHT_EN, 0);
	udelay(10);
	gpio_set_value(GPIO_LCD_BACKLIGHT_EN, 1);
...
static void ktd2599_send_high(void)
{
	gpio_set_value(GPIO_LCD_BACKLIGHT_EN, 0);
	udelay(1);
	gpio_set_value(GPIO_LCD_BACKLIGHT_EN, 1);
...
static void ktd2599_send_start(void)
{
	gpio_set_value(GPIO_LCD_BACKLIGHT_EN, 1);
	udelay(10);
}

static void ktd2599_send_end(void)
{
	gpio_set_value(GPIO_LCD_BACKLIGHT_EN, 0);
	udelay(10);
	gpio_set_value(GPIO_LCD_BACKLIGHT_EN, 1);
...
static void ktd2599_send_byte(char value)
{
	int i;
	int mask = 1 << 7;
...
static int qpnp_ktd2599_set(struct qpnp_led_data *led)
{
	int level;
	static int old_level = 0;
	static bool is_bl_on = false;

	level = led->cdev.brightness;
	if (level > LED_FULL)
		level = LED_FULL;

	level = (2 * level * 31 + LED_FULL) / (2 * LED_FULL);
	if (level == old_level)
		return 0;
	...
	if (level == 0) {
		...
		ktd2599_send_byte(KTD2599_DEVICE_ADDRESS);
		ktd2599_send_byte(level);
	}
...
static int qpnp_flash_set(struct qpnp_led_data *led)
{
	...
	/* Set led current */
	if (val > 0) {
		pr_debug("%s: name = %s, val = %d\n", __func__, led->cdev.name, val);
		if (led->flash_cfg->torch_enable) {
...
/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, FR-542791, 2013/11/16, change torch brightness */
#ifdef CONFIG_TCT_8X26_COMMON
                rc = qpnp_led_masked_write(led,
                    FLASH_ENABLE_CONTROL(led->base),
                    //FLASH_ENABLE_MASK, /*this is supplied by qct, but causes flash shines only once, replace by belowing*/
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2013/9/9, Add lcd backlight driver for IC KTD2599*/
#ifdef CONFIG_TCT_8X26_RIOL
		if (g_board_id > 1600 && g_board_id < 1700 && strcmp(led->cdev.name, "wled:backlight") == 0)
			rc = qpnp_ktd2599_set(led);
		else
			rc = qpnp_mpp_set(led);
#else
		rc = qpnp_mpp_set(led);
	...
	const char *buf, size_t count)
{
...
#ifdef CONFIG_TCT_8X26_COMMON
	uint uvalue;  /*[BUGFIX]-Add-BEGIN by TCTNB.XQJ, FR-510131 2013/9/12, modify attribute interface with  charge LED and incident LED*/
#endif
       int value;
	...
	for (i = 0; i < max_duty_pcts; i++) {
		if (buffer == NULL)
			break;
/*[BUGFIX]-Add-BEGIN by TCTNB.XQJ, FR-510131 2013/9/12, modify attribute interface with  charge LED and incident LED*/
#ifdef CONFIG_TCT_8X26_COMMON
             if (memcmp (led->cdev.name, "green", 5) == 0 )
             {
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2013/8/13, Add lcd backlight driver for IC KTD2599*/
#ifdef CONFIG_TCT_8X26_RIOL
		if (strcmp(led->cdev.name, "wled:backlight") == 0)
		{
			vadc_dev = qpnp_get_vadc(&spmi->dev, "boardid");
			if (IS_ERR(vadc_dev))
				printk("Unable to get vadc for board id\n");

			rc = qpnp_vadc_read(vadc_dev, P_MUX7_1_3, &results);
			if (rc)
				printk("Unable to read board id rc=%d\n", rc);

			g_board_id = (int)results.physical / 1000;
			printk("board id (for lcd backlight) is %d\n", g_board_id);
		}
#endif
/*[PLATFORM]-Add-END by TCTNB.WQF*/
